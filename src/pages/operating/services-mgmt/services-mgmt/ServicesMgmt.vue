<template>
  <biz-grid class="services-mgmt">
    ...
    <table-view :data="tableData" v-show="views[1].actived" @row-click="rowClick" ref="tableView">
      ...
      <el-table-column label="状态" prop="subscribeState">
        <span slot-scope="scope">
          <span :style="{color: stateColor(scope.row)}">{{ scope.row.isMarketable | formatState }}</span>
        </span>
      </el-table-column>
      <el-table-column label="上级服务" prop="endDate">
        <span slot-scope="scope">
          {{ scope.row.businessService.parents.length>0&&scope.row.businessService.parents[0].name || '--'}}
        </span>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    moveUp(row) {
      ...
      http
        .post('/admin/businessProduct/businessServiceSort.jhtml', params)
        .then(data => {
          this.getList();
          this.loading = false;
        })
