<template>
  <biz-grid class="function-defination-base-info">
    ...
    <el-form :rules="rules" ref="form" :model="form" label-width="120px" size="small" v-loading="loading" :inline-message="true">
      ...
      <el-form-item label="优惠" prop="">
        <div v-for="(promotion, index) in promotionData" :key="index"><el-tag size="mini">{{promotion.coupon.name}}</el-tag><span style="font-size:12px;margin-left:5px">{{getType(promotion.coupon)}}</span>
          <div v-if="promotion.coupon.type==='1'">
            <el-card class="box-card">
              <div style="display:flex;justify-content:space-between">
                <div>
                  <div class="text">
                    ￥{{promotion.coupon.amount}}
                  </div>
                  <div>
                    <span v-if="promotion.coupon.minimumPrice">满{{promotion.coupon.minimumPrice}}可用</span><span v-if="!promotion.coupon.minimumPrice">无门槛优惠券</span>
                  </div>
                </div>
                <div>
                  <el-button size="mini" type="primary" @click="getCoupon(promotion.coupon)">立即领取</el-button>
                </div>
              </div>
            </el-card>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      dialogVisible: false,
      operations: [{ label: '加入订阅清单', name: 'jion', type: 'primary', disabled: false, class: 'show' }, { label: '立即订阅', name: 'subscribe', type: 'primary', disabled: false }],
      rules: {
  ...
  methods: {
    ...
    getCoupon(row) {
      ...
      http.post('/admin/promotion/couponCode.jhtml', params)
        .then(data => {
    ...
    getType(row) {
      if (row.type === '11') {
        return `满${row.minimumPrice}送${row.giftProducts[0].fullName}`;
      } else if (row.type === '10') {
        return `${row.discount % 100}折`;
      } else if (row.type === '9') {
        return `满${row.minimumPrice}打${row.discount}折`;
      } else if (row.type === '9') {
        return `满${row.minimumPrice}打${row.discount}折`;
      } else if (row.type === '8') {
        return `满${row.minimumPrice}包邮`;
      }
    },
    ...
    queryPromotion() {
      ...
      http.get(this.isFn ? '/admin/businessOrder/solutionCoupons.jhtml' : '/admin/businessOrder/productCoupons.jhtml', params)
        .then(data => {
          this.promotionData = data.promotionDatas;
        })
        .catch();
    },
    goSubscribe() {
    ...
    jion() {
      let params = {
        product: { id: this.productId || this.id },
        userId: this.currentUser.id,
        quantity: '1',
        shopId: this.form.shopId
      };
      http.post('/admin/subscribeList/subscribeList.jhtml', params)
        .then(data => {
    ...
    refresh() {
      ...
      http
        .get(this.isFn ? '/admin/solution/subscribe/subscribe.jhtml' : urls.servicesMgmt.getOrderDetail, params)
        .then(data => {
          if (this.isFn) {
            this.operations[0].class = 'hidden';
            this.form = Object.assign(this.form, data.data);
            this.specificationList = data.data.specifications.sort((a, b) => {
    ...
    async subscribe() {
      ...
      try {
        if (this.form.businessServiceType === 'platform') {
          ...
          this.$emit('confirmOrderInfo', [this.form]);
        } else {
          params = {
            publishStore: this.currentUser.shop.id || window.top.SHOP_ID || '',
            specificationId: sId,
            subscribeStore: selectShop || this.currentUser.shop.id || window.top.SHOP_ID,
            products: [{ id: this.productId || this.id }]
          };
