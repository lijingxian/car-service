...
export default {
  ...
  computed: {
    ...
    selectAll() {
      ...
      if (this.selectedItemList && this.selectedItemList.length === this.list.length) {
      } else {
        return false;
      }
      return true;
    ...
    amount() {
      ...
      this.selections.forEach(arr => {
        if (arr.effectivePrice) {
          amount += Number(arr.effectivePrice);
        }
      });
  ...
  methods: {
    ...
    handleSelectAllClick() {
      if (this.selectAll) {
        ...
        this.selections = [];
        for (let i = 0; i < this.list.length; i++) {
          this.$set(this.selections, i, this.list[i]);
        }
      }
    },
    ...
    handleBuy(e) {
      ...
      this.$emit('confirmOrderInfo', this.selections);
    },
    handleFavorite(e) {
      if (this.selectedItemList.length === 0) {
        this.$message.warning('请选择一个商品', 'warn');
        ...
        .then(data => {
          ...
          if (data.dataList.length > 0) {
            for (let i = 0; i < data.dataList.length; i++) {
              total += data.dataList[i].length;
            }
          }
          this.$emit('cartCountQuery', data.dataList.length > 0 ? total : '0');
    ...
    remove(ids, item, itemIndex, shop, shopIndex) {
      ...
      http
        .delete('/admin/subscribeList/subscribeList.jhtml', {
          clientType: '2',
          userId: this.getUser.id,
          shopId: this.getUser.shop.id,
