<template>
  <div class="terminal-mgmt-terminal__people-picker">
    <el-input placeholder="请输入所有者" v-model="nameText" readonly="readonly">
...
export default {
  ...
  methods: {
    ...
    confirmedHandler(selectionKeys, selections) {
      ...
      if (this.value.nickname) {
        this.$confirm('修改所有者将会解除设备与原所有者及车辆的关联?', '提示', {
  ...
  computed: {
    nameText() {
      return this.value.name ? this.value.name : this.value.nickname ? this.value.nickname : this.value.userName;
    }
  }
};
</script>
