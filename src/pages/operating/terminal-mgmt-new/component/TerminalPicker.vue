<template>
  <div class="vehicle-mgmt-vehicle__vehicle-picker">
    ...
    <biz-dialog-selector slot="float" title="设备选择" :visibility.sync="showDialog" :loading="dialogLoading" v-if="motorcadeId"
    ...
    <biz-dialog-selector slot="float" title="设备选择" :visibility.sync="showDialog" :loading="dialogLoading" v-if="!motorcadeId"
      ...
      @confirmed="confirmedHandler" @open="openedHandler" @filterDataChange="dialogFilterDataChange"
      @pageSizeChange="dialogPageSizeChange" @currentPageChange="dialogCurrentPageChange">
      <el-table-column prop="name" label="终端名称" width="100">
      </el-table-column>
      <el-table-column prop="sn" label="序列号" width="100">
      </el-table-column>
      <el-table-column prop="originalId" label="终端分类">
      </el-table-column>
      <el-table-column prop="owner" label="所有者姓名">
      </el-table-column>
      <el-table-column prop="ownerUuid" label="所有者电话">
      </el-table-column>
    </biz-dialog-selector>
  </div>
</template>
...
export default {
  ...
  computed: {
    textComputed() {
      return '已选择 ' + this.value.length + '个设备';
    }
  },
  methods: {
