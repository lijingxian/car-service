<template>
  <biz-grid class="vehicle-mgmt-all-vehicle__vehicle-data-steam">
    ...
    <div>
      <el-form ref="form" size="small" label-width="110px" :model="formModel" :rules="rules" :disabled="!isEdit">
        <el-form-item label="检测设置" prop="option">
          <el-form ref="form" size="small" label-width="55" :model="formModel" :rules="rules" :inline="true" class="form-inline"
      ...
      <div class="biz-card-list">
        <div v-for="item in formModel.userOptions" :key="item.id" class="biz-card-list__item" :class="{ selected: item.id === selectedOption }"
      ...
      <el-form size="small" label-width="0" :model="formModel">
        <el-collapse v-model="activeNames">
          <el-collapse-item title="常用" name="1">
            <el-form-item prop="name">
              <el-checkbox-group v-model="userOptions" class="vehicle-data-steam__checkbox-group">
                <div v-for="item in optionList" :key="item.id" class="checkbox-item">
                  <el-checkbox :label="item.id" :disabled="(Number(userOptions.length) === 6 && Number(userOptions.indexOf(item.id)) < 0) || (Number(userOptions.length) === 1 && Number(userOptions.indexOf(item.id)) === 0) || checkboxDisable">
                    {{item.content}}
                  </el-checkbox>
                </div>
              </el-checkbox-group>
          ...
          <el-collapse-item title="更多" name="2">
            <el-form-item prop="name">
              <el-checkbox-group v-model="userOptions" class="vehicle-data-steam__checkbox-group">
                <div v-for="item in optionMoreList" :key="item.id" class="checkbox-item">
                  <el-checkbox :label="item.id" :disabled="(Number(userOptions.length) === 6 && Number(userOptions.indexOf(item.id)) < 0) || (Number(userOptions.length) === 1 && Number(userOptions.indexOf(item.id)) === 0) || checkboxDisable">
                    {{item.content}}
                  </el-checkbox>
                </div>
  ...
  methods: {
    ...
    query(queryType = '1') {
      let params = {
        ...
        terminalId: String(this.itemId.id),
        // carId: '133',
        queryType: queryType
      };
      service.query(
        ...
        data => {
          ...
          if (String(queryType) === '2') {
            ...
            this.optionMoreList = [];
            data.dataFlowParams.map(dataFlow => {
              this.optionMoreList.push({
                content: dataFlow.name,
                unit: dataFlow.unit,
                id: dataFlow.id,
          ...
          if (this.streamData[this.streamData.length - 1] && this.streamData[this.streamData.length - 1].datas) {
            this.streamData[this.streamData.length - 1].datas.map(data => {
              if (optionsArr.indexOf(data.pid) !== -1) {
                this.$set(
                  this.formModel.userOptions[optionsArr.indexOf(data.pid)],
                  'value',
                  data.dataType === '1' ? data.text : data.value
                );
              }
            });
          }
    ...
    addHalfSecond(str) {
      ...
      tmp = `${time.getHours() < 10 ? '0' + time.getHours() : time.getHours()}:${time.getMinutes()}:${
