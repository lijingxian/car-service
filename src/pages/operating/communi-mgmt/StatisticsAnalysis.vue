...
export default {
    ...
    return {
      ...
      filters: [
        ...
        },
        {
          ...
          value: 'increase',
          options: [
            { value: 'increase', label: '通讯卡新增数' },
            { value: 'total', label: '通讯卡累计数' }
          ]
        },
        {
          type: 'daterange',
  ...
  methods: {
    init() {
      let begin = new Date(this.filters[3].value[0]);
      let end = new Date(this.filters[3].value[1]);
    ...
    getFormatDate() {
      let date = new Date();
      this.filters[3].value[1] = `${date.getFullYear()}-${date.getMonth() < 9 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1}-${date.getDate() < 10 ? '0' + date.getDate() : date.getDate()}`;
  ...
  computed: {
    kpiName: function() {
      if (this.filters[2].value === 'increase') {
        return ['    通讯卡新增数', '留存率'];
      }
      if (this.filters[2].value === 'total') {
        return ['    通讯卡累计数', '增长率'];
      }
      return [];
    }
  ...
  mounted() {
    this.$message.info({
      message: '可在此处了解通讯卡商品的销售, 盈利情况及卡的增长情况',
      showClose: true
    });
  },
  components: {
