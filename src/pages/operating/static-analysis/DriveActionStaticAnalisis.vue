    ...
    <biz-chart :chartData="chartData" :filters="filters" @filterDataChange="filterDataChange" :loading="loading" :pagination="pagination"
      ...
      <div slot="list">
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%">
          ...
          <el-table-column prop="deceleration" label="急减速(次)">
          </el-table-column>
          <el-table-column prop="acceleration" label="急加速(次)">
          </el-table-column>
          <el-table-column prop="sharpTurn" label="急转弯(次)">
          </el-table-column>
          <el-table-column prop="rapidBrake" label="急刹车(次)">
          </el-table-column>
          <el-table-column prop="overRotateSpeed" label="高转速(次)">
          </el-table-column>
          <el-table-column prop="idleSpeed" label="长怠速(次)">
          </el-table-column>
        </biz-list>
      </div>
...
export default {
    ...
    return {
      ...
      filters: [
        ...
        },
        {
          ...
          value: 'deceleration',
          options: [
            { value: 'deceleration', label: '急减速' },
            { value: 'acceleration', label: '急加速' },
            { value: 'sharpTurn', label: '急转弯' },
            { value: 'rapidBrake', label: '急刹车' },
            { value: 'overRotateSpeed', label: '高转速' },
            { value: 'idleSpeed', label: '长怠速' }
          ]
        }
      ],
      kpiName: ['当前', '同比', '环比'],
  ...
  methods: {
    init() {
      ...
      http
        .get(urls.staticAnalisisi.driveBehaviorGraph, params)
        .then(data => {
    ...
    getTable() {
      ...
      http
        .get(urls.staticAnalisisi.driveBehaviorTable, params)
        .then(data => {
    ...
    exportExcle() {
      ...
      http
        .get(urls.staticAnalisisi.driveBehaviorExport, params)
        .then()
        .catch(data => {
          if (!data.errorMessage && data !== '没有符合条件的数据') {
            ...
            let linkedUrl =
              `${urls.staticAnalisisi.driveBehaviorExport}?` +
              paramsArr.join('&');
            // 异步弹出窗口会被Chrome拦截
