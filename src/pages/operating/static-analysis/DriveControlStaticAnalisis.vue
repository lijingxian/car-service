    ...
    <biz-chart :chartData="chartData" :filters="filters" @filterDataChange="filterDataChange" :loading="loading" :pagination="pagination"
      ...
      <div slot="list">
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%">
          ...
          <el-table-column prop="fault" label="故障（次）">
          </el-table-column>
          <el-table-column prop="collision" label="碰撞（次）">
          </el-table-column>
          <el-table-column prop="underVoltage" label="欠压（次）">
          </el-table-column>
          <el-table-column prop="highTemperature" label="高温(次)">
          </el-table-column>
          <el-table-column prop="towaway" label="被拖吊(次)">
          </el-table-column>
          <el-table-column prop="fenceinout" label="入围栏/出围栏(次)">
          </el-table-column>
          <el-table-column prop="fireinout" label="点火/熄火(次)">
          </el-table-column>
          <el-table-column prop="pluginout" label="插电/拔出(次)">
          </el-table-column>
        </biz-list>
      </div>
...
export default {
    ...
    return {
      ...
      filters: [
        ...
        },
        {
          ...
          value: 'fault',
          options: [
            { value: 'fault', label: '故障数' },
            { value: 'collision', label: '碰撞数' },
            { value: 'underVoltage', label: '欠压数' },
            { value: 'highTemperature', label: '高温数' },
            { value: 'towaway', label: '被拖吊数' },
            { value: 'fenceout', label: '出围栏数' },
            { value: 'fencein', label: '入围栏数' },
            { value: 'fire', label: '点火数' },
            { value: 'flameout', label: '熄火数' },
            { value: 'plugin', label: '插电数' },
            { value: 'plugout', label: '拔出数' }
          ]
        }
      ],
      kpiName: ['当前', '同比', '环比'],
  ...
  methods: {
    init() {
      ...
      http
        .get(urls.staticAnalisisi.driveControlGraph, params)
        .then(data => {
    ...
    getTable() {
      ...
      http
        .get(urls.staticAnalisisi.driveControlTable, params)
        .then(data => {
    ...
    exportExcle() {
      ...
      http
        .get(urls.staticAnalisisi.driveControlExport, params)
        .then()
        .catch(data => {
          if (!data.errorMessage && data !== '没有符合条件的数据') {
            ...
            let linkedUrl =
              `${urls.staticAnalisisi.driveControlExport}?` + paramsArr.join('&');
            // 异步弹出窗口会被Chrome拦截
