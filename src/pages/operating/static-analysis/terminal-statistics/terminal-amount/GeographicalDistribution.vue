  ...
  <div class="geographical-distribution">
     <biz-chart :chartData="chartData" :loading="loading" :pagination="pagination" :showViewType="false"
      ...
      <div slot="description">
        ...
        <p>2、终端启用数：在统计期开始的前30天内[30]、及统计期内，有产生过数据的设备总数。上传过GPS数据/轨迹数据/其他车辆数据等
    环比启用数：环比数据：即所选统计期（如2018年N月）与上一个连续统计期（如2017年N-1月）的数据比</p>
        <p>3、终端启用比例=终端启用数/终端总数*100%</p>
      </div>
    </biz-chart>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async exportExcle() {
      ...
      try {
        let response = await axios.post(urls.terminalMgmt.areaExport, params, { responseType: 'blob' });
    ...
    getGraphData() {
      ...
      http
        .post(urls.terminalMgmt.areaGraphData, param)
        .then(data => {
          this.loading = false;
    ...
    dataFormat(data) {
      ...
      for (let i = 0; i < data.indicators.length; i++) {
        tmpData.push({
          ...
          终端启用数: Number(data.indicators[i].y[0].yYearOnYear),
          地域启用数占比: data.indicators[i].y[0].yMonthOnMonth
        });
        maxValue = Math.max(maxValue, Number(data.indicators[i].y[0].yYearOnYear));
      }
    ...
    getTabelData() {
      ...
      let param = this.getParams();
      http
        .post(urls.terminalMgmt.areaTabelData, param)
        .then(data => {
          this.loading = false;
