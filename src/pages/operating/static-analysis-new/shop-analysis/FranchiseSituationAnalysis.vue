<template>
  <div class="franchise-situation-analysis">
    <div slot="top">
    ...
    <div class="chart-area">
      <div class="graph">
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="(filters[0].value==='0'&&this.filters[1].value==='0') && isShow"></biz-chart>
        <biz-chart :chartData="chartDataOfLine" :loading="loading" :showSearch="false" v-if="(filters[0].value!=='0'||this.filters[1].value!=='0') && isShow"></biz-chart>
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-if="!isShow">
          ...
          <el-table-column
            prop="companyName"
            label="所属运营商"
            v-if="this.filters[1].value!=='4'&&this.filters[1].value!=='5'&&this.filters[1].value!=='6'&&this.filters[1].value!=='7'"
          ></el-table-column>
          <el-table-column prop="address" label="地址"></el-table-column>
          <el-table-column prop="state" label="状态" :formatter="getStateFormatter"></el-table-column>
        </biz-list>
...
export default {
  name: 'FranchiseSituationAnalysis',
  data() {
    return {
      loading: true,
      height: '500',
      ...
      filters: [
        ...
        },
        {
          ...
          options: [
            ...
            { value: '3', label: '店铺留存率' },
            { value: '4', label: '运营商分布' },
            { value: '5', label: '运营商累计数' },
            { value: '6', label: '运营商新增数' },
            { value: '7', label: '运营商留存率' }
          ]
        }
      ],
      kpiName: ['本期', '同比', '环比'],
  ...
  methods: {
    ...
    swichShopByShop() {
      this.classNameShopByShop =
        this.classNameShopByShop === 'el-icon-caret-bottom' ? 'el-icon-caret-top' : 'el-icon-caret-bottom';
    ...
    getGraphMapData() {
      ...
      http
        ...
        .then(data => {
          this.chartData = this.dataMapFormat(data);
        })
        .catch(errData => {
          this.loading = false;
    ...
    getGraphLineData() {
      ...
      let params = {
        ...
        endDate: this.endDate,
        shopIds:
          this.getUser().roleLevel !== 'shop' ? this.checkedShop.join('&shopIds=') : this.checkedShopByShop.join('&shopIds=')
      };
      let paramsArr = [];
      for (let name in params) {
        paramsArr.push(`${name}=${params[name]}`);
      ...
      let urls = '';
      if (this.getUser().roleLevel !== 'shop') {
        if (this.filters[1].value === '1') {
          urls = '/admin/ecommerce/kpi/graph/stores/storeCumulative.jhtml';
        } else if (this.filters[1].value === '2') {
          urls = '/admin/ecommerce/kpi/graph/stores/storeAddition.jhtml';
        } else if (this.filters[1].value === '3') {
          urls = '/admin/ecommerce/kpi/graph/stores/storeRetention.jhtml';
        } else if (this.filters[1].value === '4') {
          urls = '/admin/ecommerce/kpi/table/stores/companyArea.jhtml';
        } else if (this.filters[1].value === '5') {
          urls = '/admin/ecommerce/kpi/graph/stores/companyCumulative.jhtml';
        } else if (this.filters[1].value === '6') {
          urls = '/admin/ecommerce/kpi/graph/stores/companyAddition.jhtml';
        } else if (this.filters[1].value === '7') {
          urls = '/admin/ecommerce/kpi/graph/stores/companyRetention.jhtml';
        }
      } else {
      ...
      http
        ...
        .then(data => {
          this.chartDataOfLine = this.dataLineFormat(data);
        })
        .catch(errData => {
          this.loading = false;
          if (errData.errorMessage) this.$message.warning(errData.errorMessage);
    ...
    getTable() {
      ...
      if (this.getUser().roleLevel !== 'shop') {
        if (
          ...
          this.filters[1].value === '3' ||
          this.filters[1].value === '5' ||
          this.filters[1].value === '7'
        ) {
          urls = '/admin/ecommerce/kpi/table/stores/storeAreaList.jhtml';
        } else if (this.filters[1].value === '2') {
          urls = '/admin/ecommerce/kpi/table/stores/storeCumulative.jhtml';
        } else if (this.filters[1].value === '4') {
          urls = '/admin/ecommerce/kpi/table/stores/companyAreaList.jhtml';
        } else if (this.filters[1].value === '6') {
          urls = '/admin/ecommerce/kpi/table/stores/companyAddition.jhtml';
        }
      } else {
        if (this.filters[0].value === '0' || this.filters[0].value === '1' || this.filters[0].value === '3') {
          urls = '/admin/ecommerce/kpi/table/stores/storeAreaList.jhtml';
        } else if (this.filters[0].value === '2') {
          urls = '/admin/ecommerce/kpi/table/stores/storeCumulative.jhtml';
        }
    ...
    exportStart() {
      ...
      let urls = '';
      if (this.getUser().roleLevel !== 'shop') {
        if (this.filters[1].value === '0') {
          urls = '/admin/ecommerce/kpi/export/stores/exportStoreAreaListStart.jhtml';
        } else if (this.filters[1].value === '1') {
          urls = '/admin/ecommerce/kpi/export/stores/exportStoreAreaListStart.jhtml';
        } else if (this.filters[1].value === '2') {
          urls = '/admin/ecommerce/kpi/export/stores/exportStoreCumulativeStart.jhtml';
        } else if (this.filters[1].value === '3') {
          urls = '/admin/ecommerce/kpi/export/stores/exportStoreAreaListStart.jhtml';
        } else if (this.filters[1].value === '4') {
          urls = '/admin/ecommerce/kpi/export/stores/companyAreaList.jhtml';
        } else if (this.filters[1].value === '5') {
          urls = '/admin/ecommerce/kpi/export/stores/companyAreaList.jhtml';
        } else if (this.filters[1].value === '6') {
          urls = '/admin/ecommerce/kpi/export/stores/companyAddition.jhtml';
        } else if (this.filters[1].value === '7') {
          urls = '/admin/ecommerce/kpi/export/stores/companyAreaList.jhtml';
        }
      } else {
    ...
    dataLineFormat(data) {
      ...
      let chartData = {
        ...
        settings: {
          ...
          yAxisName: this.filters[0].value === '3' || this.filters[1].value === '3' || this.filters[1].value === '7' ? ['%', '%'] : ['个', '%'],
