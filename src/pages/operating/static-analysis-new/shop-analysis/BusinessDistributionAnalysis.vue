<template>
  <div class="business-distribution-analysis">
    <div slot="top">
    ...
    <div class="chart-area">
      <div class="graph">
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;max-height:800px;overflow:auto" v-if="isShow">
          <div v-for="(list,index) in lists" :key="index">
            <span :class="'list'+index">{{list.lable}}:{{list.value}}个 占比{{countTotal!==0?((list.value/countTotal)*100).toFixed(2):list.value}}%</span>
          </div>
        </div>
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-if="!isShow">
          ...
          <el-table-column prop="companyName" label="所属运营商"></el-table-column>
          <el-table-column prop="sn" label="订单编号" v-if="this.filters[0].value==='0'"></el-table-column>
          <el-table-column prop="type" label="订单类型" v-if="this.filters[0].value==='0'"></el-table-column>
          <el-table-column prop="memberName" label="会员名称" v-if="this.filters[0].value==='1'"></el-table-column>
          <el-table-column prop="memberMobile" label="会员手机号" v-if="this.filters[0].value==='1'"></el-table-column>
          <el-table-column prop="tagName" label="活跃程度" v-if="this.filters[0].value==='1'"></el-table-column>
          <el-table-column prop="plateNumber" label="车牌号" v-if="this.filters[0].value==='2'"></el-table-column>
          <el-table-column prop="ownerName" label="车主" v-if="this.filters[0].value==='2'"></el-table-column>
          <el-table-column prop="brand" label="车辆品牌" v-if="this.filters[0].value==='2'"></el-table-column>
          <el-table-column prop="name" label="潜客信息" v-if="this.filters[0].value==='3'"></el-table-column>
          <el-table-column prop="level" label="潜客级别" v-if="this.filters[0].value==='3'"></el-table-column>
          <el-table-column prop="title" label="文章标题" v-if="this.filters[0].value==='4'"></el-table-column>
          <el-table-column prop="type" label="文章类型" v-if="this.filters[0].value==='4'"></el-table-column>
          <el-table-column prop="name" label="活动标题" v-if="this.filters[0].value==='5'"></el-table-column>
          <el-table-column prop="type" label="活动类型" v-if="this.filters[0].value==='5'"></el-table-column>
        </biz-list>
...
export default {
  name: 'BusinessDistributionAnalysis',
  data() {
    return {
      loading: true,
      height: '500',
  ...
  methods: {
    ...
    getGraphData() {
      ...
      if (this.filters[0].value === '0') {
        urls = '/admin/ecommerce/kpi/graph/store/storeOrderType.jhtml';
      } else if (this.filters[0].value === '1') {
        urls = '/admin/ecommerce/kpi/graph/store/storeMemberActive.jhtml';
      } else if (this.filters[0].value === '2') {
        urls = '/admin/ecommerce/kpi/graph/store/storeCarBrand.jhtml';
      } else if (this.filters[0].value === '3') {
        urls = '/admin/ecommerce/kpi/graph/store/pie.jhtml';
      } else if (this.filters[0].value === '4') {
        urls = '/admin/ecommerce/kpi/graph/store/storesArticlePreference.jhtml';
      } else if (this.filters[0].value === '5') {
        urls = '/admin/ecommerce/kpi/graph/store/storesActivityType.jhtml';
      }
      http
        .get(urls + '?' + paramsArr.join('&'))
        .then(data => {
          if (this.filters[0].value === '3') {
            this.lists = data.data;
            this.countTotal = 0;
            for (const item of data.data) {
              this.countTotal += Number(item.value);
            }
            this.chartData = this.dataFormat(data.data);
          } else {
            this.lists = data.indicators;
            this.countTotal = 0;
    ...
    getTable() {
      ...
      let urls = '';
      if (this.filters[0].value === '0') {
        urls = '/admin/ecommerce/kpi/table/store/storeOrderType.jhtml';
      } else if (this.filters[0].value === '1') {
        urls = '/admin/ecommerce/kpi/table/store/storeMemberActive.jhtml';
      } else if (this.filters[0].value === '2') {
        urls = '/admin/ecommerce/kpi/table/store/storeCarBrand.jhtml';
      } else if (this.filters[0].value === '3') {
        urls = '/admin/ecommerce/kpi/table/store/pie.jhtml';
      } else if (this.filters[0].value === '4') {
        urls = '/admin/ecommerce/kpi/table/store/storesArticlePreference.jhtml';
      } else if (this.filters[0].value === '5') {
        urls = '/admin/ecommerce/kpi/table/store/storesActivityType.jhtml';
      }
      http
        .get(urls + '?' + paramsArr.join('&'))
        .then(data => {
          if (this.filters[0].value === '1') {
            this.tableData = data.content;
          } else {
            this.tableData = data.indicators;
            if (this.filters[0].value === '0') {
              for (const item of this.tableData) {
                item.shopName = item.sellShopName;
              }
            }
          }
    ...
    exportStart() {
      ...
      let urls = '';
      if (this.filters[0].value === '0') {
        urls = '/admin/ecommerce/kpi/export/store/exportStoreOrderTypeStart.jhtml';
      } else if (this.filters[0].value === '1') {
        urls = '/admin/ecommerce/kpi/export/store/exportStoreMemberActiveStart.jhtml';
      } else if (this.filters[0].value === '2') {
        urls = '/admin/ecommerce/kpi/export/store/exportStoreCarBrandStart.jhtml';
      } else if (this.filters[0].value === '3') {
        urls = '/admin/ecommerce/kpi/export/store/exportStorePieGraphStart.jhtml';
      } else if (this.filters[0].value === '4') {
        urls = '/admin/ecommerce/kpi/export/store/exportStoresArticlePreferenceStart.jhtml';
      } else if (this.filters[0].value === '5') {
        urls = '/admin/ecommerce/kpi/export/store/exportStoresActivityTypeStart.jhtml';
      }
      http
        .post(urls, params)
        .then(data => {
