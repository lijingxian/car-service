  ...
  <div class="action-analysis">
    ...
    <div class="chart-area">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="safetyIndex">安全指数</el-radio-button>
              <el-radio-button label="oilIndex">油耗指数</el-radio-button>
              <el-radio-button label="useRate">用车频率</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <biz-chart :chartData="chartDataOfOil" :loading="loading" :showSearch="false"
          v-if="radioValue==='0'&&indicatorValue==='oilIndex'"></biz-chart>
        <biz-chart :chartData="chartDataOfUseRate" :loading="loading" :showSearch="false"
          v-if="radioValue==='0'&&indicatorValue==='useRate'"></biz-chart>
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='safetyIndex'">
        </biz-chart>
      <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%;height:100%" v-show="radioValue==='1'">
          ...
          <el-table-column prop="avgOil" label="平均油耗"></el-table-column>
          <el-table-column prop="drivingHabit" label="驾驶习惯"></el-table-column>
          <el-table-column prop="safetyIndex" label="驾驶类型"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    singleCar(row) {
      this.$router.push({
        path: '/car/statistics-analysis/vehicleStatic/singleViewAnalysis',
    ...
    indicatorChanged() {
      if (this.indicatorValue === 'oilIndex') {
        this.getGraphOilData();
      } else if (this.indicatorValue === 'useRate') {
        this.getGraphUseRateData();
      } else {
        this.getGraphData();
      }
    ...
    getGraphUseRateData() {
      ...
      http
        ...
        .then(data => {
          this.chartDataOfUseRate = this.dataFormat(data);
        })
        .catch(errData => {
          this.loading = false;
    ...
    getGraphOilData() {
      ...
      http
        .post('/admin/ecommerce/kpi/car/graph/drivingAction.jhtml', params)
        .then(data => {
          this.chartDataOfOil = this.dataFormat(data);
        })
        .catch(errData => {
          this.loading = false;
          if (errData.errorMessage) this.$message.warning(errData.errorMessage);
    ...
    exportStart() {
      ...
      http
        .post('/admin/ecommerce/kpi/car/export/drivingActions/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/ecommerce/kpi/car/export/drivingActions/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
    ...
    dataFormat(data) {
      ...
      for (let i = 0; i < data.indicators.length; i++) {
        tmpData.push({
          ...
          分布情况: data.indicators[i].y[0].yValue
        });
      }
      let chartData = {
        legendVisible: false,
