  ...
  <div class="operate-report">
    ...
    <div ref="imageWrapper">
      <div class="title-header">
        <span>{{shopName}}</span><span>运营月报</span><span>|</span><span>{{endDate.split('-')[0]+'年'+endDate.split('-')[1]+'月'}}1-{{lastDate}}日</span>
      </div>
      <div class="content">
        <div class="title-header">
          <span>一、销售情况</span>
        ...
        <div class="des">
          <p>
            ...
            共新增订单{{indicators.orderInc}}单，总销售额{{indicators.orderAmountInc}}万元，同比增加{{indicators.orderAmountIncYoy}}%，环比增加{{indicators.orderAmountIncMom}}%；
            <!-- 其中{{indicators.orderProductAvgNumInc}}销售额增长迅速，同比增加{{indicators.orderProductAvgNumIncYoy}}%；
            {{indicators.orderAmountPaidInc}}销售额增长大幅下降，下降幅度超过{{indicators.orderAmountPaidIncYoy}}%。 -->
          </p>
          <p>
            从销售热度分析，整车商品中{{(carData.length>0&&carData[0].xValue)?carData[0].xValue:''}}最受欢迎；
            车品中{{(productData.length>0&&productData[0].xValue)?productData[0].xValue:''}}最受欢迎；服务中{{(serviceData.length>0&&serviceData[0].xValue)?serviceData[0].xValue:''}}最受欢迎。
          </p>
        </div>
        <div class="chart-area">
          <div class="graph">
            <biz-chart :chartData="chartData" :loading="loading" :showSearch="false"></biz-chart>
            <div align="center" class="des">整车销量排行</div>
          </div>
          <div class="graph">
            <biz-chart :chartData="chartDataOfAppreciation" :loading="loading" :showSearch="false"></biz-chart>
            <div align="center" class="des">车品销量排行</div>
          </div>
          <div class="graph">
            <biz-chart :chartData="chartServiceData" :loading="loading" :showSearch="false"></biz-chart>
            <div align="center" class="des">服务销量排行</div>
          </div>
        </div>
      </div>
      <div class="content">
        ...
        <div class="des">
          <p>
            {{endDate.split('-')[0]+'年'+endDate.split('-')[1]+'月'}}，服务态度满意度{{indicators.serviceAttitudeInc}}分，同比增加{{indicators.serviceAttitudeIncYoy}}%，环比增加{{indicators.serviceAttitudeIncMom}}%；
            专业技能满意度{{indicators.professionalInc}}分，同比增加{{indicators.professionalIncYoy}}%，环比增加{{indicators.professionalIncMom}}%。
            售后次数{{indicators.afterSaleRefundNumInc}}次，同比增加{{indicators.afterSaleRefundNumIncYoy}}%，环比增加{{indicators.afterSaleRefundNumIncMom}}%。
            <span v-if="indicators.memberIncMon">以{{indicators.memberIncMom}}原因为主。售后金额{{indicators.afterSaleRefundAmountInc}}次，同比增加{{indicators.afterSaleRefundAmountIncYoy}}%，环比增加{{indicators.afterSaleRefundAmountIncMom}}%，其中进行售后次数最多的商品是{{topConsultantX[0]}}。</span>
          </p>
        </div>
        <div class="chart-area">
          <div class="graph">
            <biz-chart :chartData="chartDataOfSatisfaction" :loading="loading" :showSearch="false"></biz-chart>
            <div align="center" class="des">订单满意度</div>
          </div>
          <div class="graph">
            <biz-chart :chartData="chartDataOfConsultant" :loading="loading" :showSearch="false"></biz-chart>
            <div align="center" class="des">TOP售后商品排行</div>
          </div>
        </div>
      </div>
      <div class="content">
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      http
        .post('/admin/ecommerce/kpi/order/summary/order/report.jhtml', param)
        .then(data => {
    ...
    getGraphServiceData() {
      ...
      http
        .post('/admin/ecommerce/kpi/order/graph/order/order.jhtml', params)
        .then(data => {
          this.chartServiceData = this.dataFormat(data);
          this.serviceData = data.indicators;
        })
        .catch(errData => {
          this.loading = false;
          if (errData.errorMessage) this.$message.warning(errData.errorMessage);
    ...
    getGraphAppreciationData() {
      ...
      http
        .post('/admin/ecommerce/kpi/order/graph/order/order.jhtml', params)
        .then(data => {
          this.chartDataOfAppreciation = this.dataFormat(data);
          this.productData = data.indicators;
        })
        .catch(errData => {
          this.loading = false;
          if (errData.errorMessage) this.$message.warning(errData.errorMessage);
