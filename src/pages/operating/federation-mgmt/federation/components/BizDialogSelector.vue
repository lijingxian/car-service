<template>
  <el-dialog class="biz-dialog-selector" :title="title" :visible.sync="dialogVisible" :append-to-body="appendToBody" @close="close"
    @open="open" custom-class="biz-store-dialog-selector">
    <biz-flex-table ref="table" :operations="operations" :filters="filters" :tags="tags" :pagination="pagination" :tableData="tableData"
      :loading="loading" :checkable="checkable" @operate="operate" @row-click="rowClick" @filter-data-change="filterDataChange"
      @select-all="handleSelectAll" @select="handleSelectionChange" @size-change="pageSizeChange" @current-change="currentPageChange">
      <slot></slot>
    </biz-flex-table>
