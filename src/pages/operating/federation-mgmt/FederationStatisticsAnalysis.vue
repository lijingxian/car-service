<template>
  <div class="federation-statistics-analysis">
    <biz-chart :chartData="chartData" :filters="filters" @filterDataChange="filterDataChange" :loading="loading" :showSearch="false"></biz-chart>
  </div>
</template>

<script>
import BizChart from '@/components/BizChart';
import service from '@/service/federation-mgmt/federationMgmt.js';

export default {
  name: 'CouponKpiReport',
  data() {
    return {
      // 今天
      today: '',
      ...
      filters: [
        ...
        },
        {
          ...
          label: '统计指标',
          value: 'total',
          options: [{ value: 'total', label: '运营商累计数' }, { value: 'increase', label: '运营商新增数' }]
        },
        {
          type: 'daterange',
          label: '日期区间：',
    ...
    dataFormat(data) {
      ...
      for (let i = 0; i < data.histogram.length; i++) {
        tmpData.push({
          ...
          [this.kpiName[1]]:
            data.lineChat.length === 0 ? '' : data.lineChat[i].yValue === '0' ? '0' : data.lineChat[i].yValue / 100
        });
      }
      let chartData = {
        type: 'histogram',
...
<style lang="scss">
.federation-statistics-analysis {
  height: 100%;
  overflow: auto;
