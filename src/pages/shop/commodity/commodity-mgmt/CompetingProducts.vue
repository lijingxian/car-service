<template>
  <biz-grid class="propetry-base-info">
    ...
    <el-form ref="form" :model="form" label-width="70px" size="small" :disabled="!isEdit">
      <el-form-item label="商品竞品">
        <el-button type="primary" size="mini" @click="addCompetitor">添加</el-button>
      </el-form-item>
      <div v-for="competitor in competitorList" :key="competitor.id" style="padding:10px 0px;font-size:14px">
        <div>
          <span style="vertical-align: sub;">{{competitor.name}}</span>
          <div style="float:right;margin-bottom: 10px;">
            <el-button type="primary" size="mini" @click="editCompetitor(competitor)">添加关键词</el-button>
            <i class="el-icon-delete iconfont" @click.stop="deleteCompetitor(competitor)"></i>
          </div>
        </div>
        <div>
          <el-table size="small" :data="competitor.labels">
...
export default {
  ...
  components: { BizSaveButton, BizGrid, BizFileUploader, ImgUploader, StoreVehiclePicker },
  created() {},
  computed: {},
  methods: {
    checkVideo(msg) {
