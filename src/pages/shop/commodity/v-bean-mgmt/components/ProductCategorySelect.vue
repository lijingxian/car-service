<template>
  <div class="product-category-select">
    <el-form label-width="130px" size="small" :inline-message="true">
      ...
      <el-form-item label="品牌：">
        <el-select :value="brandId" placeholder="请输入品牌" @change="brandChange" style="width: 80%;" :disabled="true">
          <el-option v-for="item in brandOptions" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="系列：">
        <el-select :value="seriesId" placeholder="请输入系列" @change="seriesChange" style="width: 80%;" :disabled="true">
          <el-option v-for="item in seriesOptions" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="名称：" :required="true">
        <el-autocomplete :value="name" :fetch-suggestions="querySearchAsync" placeholder="请输入名称" @input="onInputName" @select="selectName" :disabled="true"></el-autocomplete>
      </el-form-item>
    </el-form>
  </div>
</template>
