  ...
  <div class="marketing-statistics-activity-kpi-report">
    <biz-chart :chartData="chartData" :filters="filters" @filterDataChange="filterDataChange" :loading="loading" :pagination="pagination"
      ...
      <div slot="description">
        <p>当期订单数：统计期内订单状态变更为“交易完成”的订单的,订单类型为服务的订单数量</p>
        <p>累计订单数：自然年内，截止至统计选择期，订单类型为服务、订单状态变更为“交易完成”的订单数总计</p>
        <p>当期订单金额：统计期内订单状态变更为“交易完成”&订单类型为服务的订单的实付金额总计—统计期内退款金额总计</p>
        <p>累计订单金额：自然年内，截止至统计选择期，订单类型为服务、订单状态变更为“交易完成”的订单金额总计</p>
      </div>
    </biz-chart>
  </div>
</template>
