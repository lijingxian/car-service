<template>
  <div class="order-mgmt-list-f">
      ...
      <el-table-column prop="date" label="操作" min-width="80">
        <template #default="scope">
          <biz-column-operation :row="scope.row" v-if="(scope.row.unionConfig.editable === '1' || getUser().roleLevel !== 'shop') && scope.row.isValid === '1'">
            <el-button type="text" size="mini" v-if="scope.row.type==='1'" @click.stop="confirmOrder(scope.row)">确认</el-button>
            <el-button type="text" size="mini" v-if="scope.row.type==='9'" @click.stop="refundsHandler(scope.row)">退款</el-button>
            <el-button type="text" size="mini" v-if="scope.row.type==='2'" @click.stop="service(scope.row)">服务</el-button>
            <el-button
              type="text"
              size="mini"
              v-if="(scope.row.type==='2'&&scope.row.method==='1')||(scope.row.type==='2'&&scope.row.method==='2')"
              @click.stop="serviceHandler(scope.row)"
            >服务</el-button>
            <el-button type="text" size="mini" v-if="scope.row.type==='3'&&scope.row.method==='0'" @click.stop="excuteHandler(scope.row)">服务</el-button>
            <el-button
              type="text"
              size="mini"
              v-if="((scope.row.type==='3'&&scope.row.method==='0')||(scope.row.type==='2'&&scope.row.method==='1'))&&(orderType==='car'||orderType==='product'||orderType==='general')"
              @click.stop="shippingHandler(scope.row)"
            >发货</el-button>
            <el-button
              type="text"
              size="mini"
              v-if="(scope.row.type==='4'&&scope.row.method==='1')||(scope.row.type==='4'&&scope.row.method==='2')"
              @click.stop="paymentHandler(scope.row)"
            >付款</el-button>
            <el-button
              type="text"
              size="mini"
              v-if="scope.row.type!=='3'&&scope.row.type!=='5'&&scope.row.type!=='6'&&scope.row.type!=='7'"
            ...
            <el-button
              type="text"
              size="mini"
              v-if="showInvoice&&getInvoiceStatus(scope.row)==='未申请'&&scope.row.type!=='6'"
    ...
    <el-popover ref="confirmOrder" placement="top-start" min-width="300" v-model="visiable">
      <p>
        <i class="el-icon-warning" style="color:#f98700;margin-right:10px"></i>您要确认此订单么?
      </p>
          ...
          <el-row :gutter="10" class="row">
            ...
            <el-col :span="10">
              <div class="grid-content bg-purple">
                使用券：
                <span v-for="(couponCodes,index) in form.couponCodes" :key="index" v-if="form.couponCodes && form.couponCodes.length > 0">{{couponCodes.name}}</span>
              </div>
            </el-col>
          </el-row>
...
import FinanceBaseInfo from '@/pages/sales/insurance-order/BaseInfo';
