<template>
  <div class="search-property">
    <el-radio-group v-model="date" @change="dateSwich(date)" size="mini">
      ...
      <el-radio-button label="month">近3月</el-radio-button>
...
export default {
    ...
    defaultDate: {
      ...
      set(newValue) {
        if (newValue) {
          ...
          this.beginDate = dateTime.getPreThreeMonthDay();
  ...
  methods: {
    getFormatDate() {
      return [dateTime.getPreThreeMonthDay(), dateTime.today()];
    },
    dateChange(value) {
    ...
    dateSwich(type) {
      switch (type) {
        case 'day':
          this.viewType = 'day';
          this.defaultDate = [dateTime.getBeforeWeek(), dateTime.today()];
          this.$emit('refreshHandle', {
            date: this.defaultDate,
            viewType: this.viewType,
            dateType: 'day'
        ...
        case 'week':
          this.viewType = 'week';
          this.defaultDate = [dateTime.getPreOneMonthDay(), dateTime.today()];
          this.$emit('refreshHandle', {
            date: this.defaultDate,
            viewType: this.viewType,
            dateType: 'week'
        ...
        case 'month':
          this.viewType = 'month';
          this.defaultDate = [dateTime.getPreThreeMonthDay(), dateTime.today()];
          this.$emit('refreshHandle', {
            date: this.defaultDate,
            viewType: this.viewType,
            dateType: 'month'
