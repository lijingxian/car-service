<template>
  <biz-grid class="order-log-info">
    <el-steps direction="vertical" space="auto" v-if="paymentMethodName!=='在线支付'">
      <el-step title="新订单" :status="active==='1'?'finish':'wait '" :class="(this.currentState==='create'||this.currentState==='confirm')?'style-isfinish':'style-new'">
      ...
      <el-step title="执行" :status="active2==='1'?'finish':'wait '" :class="(this.currentState==='service'||this.currentState==='shipping')?'style-isfinish':'style-new'">
        <div slot="description">
          ...
          <div v-for="orderLog in orderLogs" :key="orderLog.id" v-if="orderLog.type==='shipping'">
            ...
            <div class="devProcess" ref="devProcess" v-if="shippings&&shippings.shippingTraces">
              ...
              <div v-if="shippings.shippingTraces.length === 0" style="text-align:center;">暂无轨迹</div>
            </div>
          </div>
        </div>
      </el-step>
        ...
        <div slot="description">
          <div v-for="orderLog in orderLogs" :key="orderLog.id" v-if="orderLog.type==='payment'">
            ...
            <div v-if="payments&&payments.length===2">
              ...
              <p>
                支付方式：{{payments[0].paymentMethod}}
              </p>
              <p>
                买家已付款
              </p>
              <p>
                付款方式：{{payments[0].paymentType}}
              </p>
              <p>
                {{payments[0].paymentDate}}
              </p>
