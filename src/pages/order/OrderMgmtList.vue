<template>
  <div class="order-mgmt-list">
    ...
    <biz-flex-table ref="table" :operations="operations" @operate="operate" :filters="filters"
      @filter-data-change="filterDataChange" :tableData="tableData" :checkable="false" :loading="loading" :pagination="pagination"
      @selection-change="handleSelectionChange" @row-click="rowClick" @size-change="handleSizeChange"
      @current-change="handleCurrentChange" class="order-mgmt-tabel">
      ...
      <!-- <el-table-column v-for="col of columns" :key="col.id" :label="col.name" :min-width="getWidth(col)">
        <template slot-scope="scope">
          <template v-if="tableId === '8'">
            ...
            <div v-else-if="col.id === '2'" style="display:flex;justify-content:space-between;width:100%">
            ...
            <div v-else-if="col.id === '4'">￥{{scope.row.amount}}</div>
            <div v-else-if="col.id === '5'" class="customer-info" @click.stop="memberClick(scope.row)">
              <img v-if="scope.row.member.head" :src="scope.row.member.head" />
              <img v-else src="../../assets/images/default_user.png" />
              <div class="wrapper">
                <div class="titleMember" :title="scope.row.member.name">{{scope.row.member.name}}</div>
            ...
            <div v-else-if="col.id === '6'">{{scope.row.shop.name}}</div>
            <div v-else-if="col.id === '7'">{{getInvoiceStatus(scope.row)}}</div>
            <div v-else-if="col.id === '8'">{{getOrderStatus(scope.row)}}</div>
          </template>
          <template v-else-if="tableId === '13'||tableId === '14'||tableId === '15'">
            <div class="biz-two-column" v-if="col.id === '1'">
              <div class="customer-info">{{scope.row.sn}}</div>
              <i class="iconfont biz-icon-i-web-md" style="font-size:32px;color:#ff7300"
                v-if="scope.row.dType==='purchaseOrder'"></i>
            </div>
            <div v-else-if="col.id === '2'" class="customer-info" @click.stop="memberClick(scope.row)">
              <img v-if="scope.row.member.head" :src="scope.row.member.head" />
              <img v-else src="../../assets/images/default_user.png" />
              <div class="wrapper">
                <div class="titleMember" :title="scope.row.member.name">{{scope.row.member.name}}</div>
            ...
            <div v-else-if="col.id === '3'">{{scope.row.shop.name}}</div>
            <div v-else-if="col.id === '4'">{{getOrderStatus(scope.row)}}</div>
            <div v-else-if="col.id === '5'" style="display:flex;justify-content:space-between;width:100%">
              <div style="width:100%;margin-right:30px">
                <div v-for="orderItem in scope.row.orderItems" :key="orderItem.id" class="product-info">
                  <div class="titleClass" :title="orderItem.name">{{orderItem.appointment.product.fullName}}</div>
                </div>
              </div>
            </div>
          </template>
      ...
      <el-table-column prop="date" label="操作" width="120">
         <template #default="scope">
          <biz-column-operation :row="scope.row" v-if="(scope.row.unionConfig.editable === '1' || getUser().roleLevel !== 'shop') && scope.row.isValid === '1'">
            <el-button type="text" size="mini" v-if="scope.row.type==='1'&&scope.row.dType==='purchaseOrder'"
              @click.stop="confirmOrder(scope.row)">确认</el-button>
            <el-button type="text" size="mini" v-if="scope.row.type==='9' && scope.row.hasSyncOrder === '0'"
              @click.stop="refundsHandler(scope.row)">售后中</el-button>
            ...
            <el-button type="text" size="mini"
              ...
              @click.stop="excuteHandler(scope.row)">服务</el-button>
            <el-button type="text" size="mini"
              v-if="((scope.row.type==='3'&&scope.row.method==='0')||(scope.row.type==='2'&&scope.row.method==='1'))&&(orderType==='car'||orderType==='product'||orderType==='general'||scope.row.dType==='purchaseOrder') && scope.row.hasSyncOrder === '0' && orderType !== 'service'"
              @click.stop="shippingHandler(scope.row)">发货</el-button>
            <el-button type="text" size="mini"
              v-if="(scope.row.type==='4'&&scope.row.method==='1')||(scope.row.type==='4'&&scope.row.method==='2') && scope.row.hasSyncOrder === '0'"
            ...
            <el-button type="text" size="mini" v-if="showInvoice&&getInvoiceStatus(scope.row)==='未申请'&&scope.row.type!=='6'"
              @click.stop="invoiceHandler(scope.row)">补开发票</el-button>
    ...
    <biz-popup-tabs :open="open" @close="open=false" v-model="tabName">
      ...
      <el-tab-pane label="商品信息" name="productinfo"
        v-if="this.$route.params.orderType==='service'||this.$route.params.orderType==='product'||this.$route.params.orderType==='car'||this.$route.params.orderType==='general'||this.$route.params.orderType==='rescue'||this.$route.params.orderType==='couponOrder'">
        <product-info :itemId="itemId" :isEdit="isEdit" ref="productinfo" @setOrderItems="setOrderItems"
          @operationSuccess="operationSuccess"></product-info>
      </el-tab-pane>
      <el-tab-pane label="订单进度" name="loginfo" v-if="itemId">
        <order-logs :itemId="itemId" :isEdit="isEdit" :open="open" ref="loginfo" :paymentMethodName="paymentMethodName"
          @logistics="logistics"></order-logs>
      </el-tab-pane>
      <el-tab-pane label="售后进度" name="afterprocess"
        v-if="itemId&&(this.$route.params.orderType==='service'||this.$route.params.orderType==='product'||this.$route.params.orderType==='car'||this.$route.params.orderType==='general'||this.$route.params.orderType==='rescue')">
      ...
      <el-tab-pane label="服务记录" name="orderhistory"
        v-if="itemId&&(this.$route.params.orderType==='service'||this.$route.params.orderType==='rescue')">
      ...
      <vue-easy-print tableShow ref="easyPrint">
        <div class="d-p-wrap">
          ...
          <el-row :gutter="10" class="row">
            ...
            <el-col :span="10">
              <div class="grid-content bg-purple">
                使用券：
                <span v-for="(couponCodes,index) in form.couponCodes" :key="index"
                  v-show="form.couponCodes && form.couponCodes.length > 0">{{couponCodes.name}}</span>
              </div>
            </el-col>
          </el-row>
          <el-row :gutter="10" class="row" v-if="form.orderType==='1'||(form.giftAmount!=='0'&&form.giftAmount!==undefined)">
            ...
            <el-col :span="10">
              <div class="grid-content bg-purple">
                {{form.giftAmount!=='0'&&form.giftAmount!==undefined?'赠送金额：'+form.giftAmount:''}}</div>
            </el-col>
          </el-row>
      ...
      <vue-easy-print tableShow ref="easyPrint">
        <div class="d-p-wrap">
          ...
          <el-row :gutter="10" class="row" v-if="form.orderType === '8'">
            <el-col :span="14">
              <div class="grid-content bg-purple">
                体验时间：{{form.orderItems[0].appointment.driveStartDate}}-{{form.orderItems[0].appointment.driveEndDate}}</div>
            </el-col>
            <el-col :span="10">
              <div class="grid-content bg-purple">
                体验里程：{{form.orderItems[0].appointment.endMileage-form.orderItems[0].appointment.startMileage}}km</div>
            </el-col>
          </el-row>
          <el-row :gutter="10" class="row" v-if="form.orderType === '8'">
            <el-col :span="14">
          ...
          <el-row :gutter="10" class="row" v-if="form.orderType === '6'">
            <el-col :span="14">
              <div class="grid-content bg-purple">
                心仪车型：{{form.orderItems[0].appointment.product.brandName + form.orderItems[0].appointment.product.fullName}}</div>
            </el-col>
            <el-col :span="10">
              <div class="grid-content bg-purple">备注：{{form.remarks}}</div>
            </el-col>
...
export default {
  ...
  methods: {
    ...
    getTableId() {
      console.log('orderMgmt getTableId');
      if (!this.$options.name === 'OrderMgmtList') {
        return;
      }
    ...
    listAdminByRole() {
      ...
      service.listAdminByRole(
        ...
        data => {
          this.loading = false;
          this.filters[1].options = data.dataList.map(item => { return { label: item.name, value: item.id }; });
        },
        ErrorData => {
    ...
    exportStart() {
      if (this.filters[5].value[0] && this.filters[5].value[0]) {
        let firsttime = Date.parse(this.filters[5].value[0] + ' ' + '00:00:00');
        let secondtime = Date.parse(this.filters[5].value[1] + ' ' + '23:59:59');
        // 2.获取时间间隔
        let timespan = secondtime - firsttime;
        // 3.获取天
        let day = Math.floor(timespan / 1000 / 60 / 60 / 24);
        console.log(day);
        if (day > 90) {
          this.$message.warning('订单单次导出日期间隔不能大于90天');
          return;
        }
      } else {
      ...
      let params = {
        ...
        shopIds: this.filters[0].value || window.top.SHOP_ID || '',
        consultantIds: this.filters[1].value || '',
        states: this.filters[2].value || '',
        invoiceStates: this.filters[3].value || '',
      ...
      http
        .post('/admin/order/orderExport/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/order/orderExport/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
    ...
    getList() {
      let params = {
        ...
        shopIds: this.filters[0].value.join('&shopIds='),
        dateType: this.filters[4].value,
        searchValue: this.filters[6].value,
        orderType: this.orderType,
        consultantIds: this.filters[1].value.join('&consultantIds='),
        states: this.filters[3].value.join('&states='),
        invoiceStates: this.filters[2].value.join('&invoiceStates='),
        beginTime: this.filters[5].value[0] ? this.filters[5].value[0] + ' ' + '00:00:00' : '',
        endTime: this.filters[5].value[1] ? this.filters[5].value[1] + ' ' + '23:59:59' : '',
      ...
      http
        .get('/admin/order/orderList.jhtml' + '?' + paramsArr.join('&'))
        .then(data => {
          this.loading = false;
