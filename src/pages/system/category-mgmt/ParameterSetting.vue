<template>
  <biz-grid class="category-mgmt__parameter-setting">
      ...
      <el-table-column prop="title" label="操作" width="200">
        <template slot-scope="scope">
          <div class="biz-customcol">
            <el-button type="text" size="mini" @click.stop="scope.row.id===parameterDataList[0].id?'':moveUp(scope.row)">上移</el-button>
            <el-button type="text" size="mini" @click.stop="scope.row.id===parameterDataList[parameterDataList.length-1].id?'':moveDown(scope.row)">下移</el-button>
    ...
    <el-dialog :visible.sync="showParameterDataEditor" center append-to-body width="500px">
      <el-form ref="parameterDataForm" size="small" label-width="110px" :model="parameterData" :rules="parameterDataFormRules">
      ...
      <el-form ref="parameterInfoForm" size="small" label-width="110px" :model="parameterInfo" :rules="parameterInfoFormRules" v-show="showParameterInfoEditor">
...
export default {
  ...
  methods: {
    ...
    showUp(row) {
      if (this.parameterDataList[0] && row.id === this.parameterDataList[0].id) {
        return false;
      } else {
        return true;
      }
    ...
    showDown(row) {
      if (this.parameterDataList[0] && row.id === this.parameterDataList[this.parameterDataList.length - 1].id) {
        return false;
      } else {
        return true;
      }
    ...
    moveUp(row) {
      this.loading = true;
      let sort = row.sort;
      this.parameterDataList.map(item => {
        if (item.id === row.id) {
          this.parameterDataList[this.parameterDataList.indexOf(row)].sort = this.parameterDataList[this.parameterDataList.indexOf(row) - 1].sort;
          this.parameterDataList[this.parameterDataList.indexOf(row) - 1].sort = sort;
          this.dataList = [this.parameterDataList[this.parameterDataList.indexOf(row)], this.parameterDataList[this.parameterDataList.indexOf(row) - 1]];
          this.sort();
        }
      });
    },
    ...
    sort() {
      http
        .put('/admin/parameter_group/parameterGroupSort.jhtml', this.dataList)
        .then(data => {
          this.query();
          this.loading = false;
        })
    ...
    rowEditClickHandler(row) {
      this.$refs.parameterDataForm.clearValidate();
