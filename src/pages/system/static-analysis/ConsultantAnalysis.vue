    ...
    <div class="chart-area">
      <div class="graph">
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-if="!isShow">
          <el-table-column prop="xValue" label="顾问"></el-table-column>
          <el-table-column prop="yValue" label="销售台次（台）" v-if="filters[0].value==='consultantOrderProductNum'"></el-table-column>
          <el-table-column prop="yValue" label="销售金额（万元）" v-if="filters[0].value!=='consultantOrderProductNum'"></el-table-column>
        </biz-list>
...
export default {
      ...
      filters: [
        {
          type: 'single',
          label: '',
          value: 'consultantOrderProductNum',
          options: [
            { value: 'consultantOrderProductNum', label: '销售台次', type: 'car' },
            { value: 'consultantOrderAmountPaid', label: '销售金额', type: 'car' }
          ]
        }
      ],
      kpiName: ['本期', '同比', '环比'],
  ...
  methods: {
    ...
    async exportExcle() {
      ...
      try {
        let response = await axios.post('/admin/ecommerce/kpi/sales/export/rank.jhtml', params, { responseType: 'blob' });
    ...
    dataFormat(data) {
      let tmpData = [];
      let y = '台';
      if (this.filters[0].value === 'consultantOrderProductNum') {
        y = '台';
