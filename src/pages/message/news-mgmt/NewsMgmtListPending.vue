<template>
  <div class="news-mgmt">
    ...
    >
      <el-table-column prop="title" label="消息" min-width="170">
      ...
      <el-table-column prop="content" label="处理情况" min-width="170">
        <template slot-scope="scope">
          <div>
            <div>{{ scope.row.verifyState === '0' ? '未处理' : '已处理' }}</div>
            <div class="overflow-style">{{scope.row.dealIngWithDate || '--'}}</div>
            <div class="overflow-style">{{scope.row.dealingWithAdmin?scope.row.dealingWithAdmin.name : '--'}}</div>
            <div class="overflow-style">{{scope.row.remarks || '--'}}</div>
          </div>
        </template>
      </el-table-column>
