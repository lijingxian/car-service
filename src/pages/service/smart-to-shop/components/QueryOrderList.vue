<template>
  <biz-dialog-selector class="query-order-list" selectionMode="single" :visibility.sync="showDialog" :tableData="tableData"
    ...
    <el-table-column label="实付金额" prop="amountPaid"></el-table-column>
  </biz-dialog-selector>
</template>
  ...
  methods: {
    ...
    confirm(keys, items) {
      ...
      let orderInfo = {
        orderId: items[0].id,
        orderSn: items[0].sn,
        productName: items[0].orderItems[0].fullName,
        amount: items[0].amount,
        amountPaid: items[0].amountPaid
      };
    ...
    refresh() {
      ...
      http
        .get(customerService.ReceptionMgmt.baseInfo.queryOrderList, params)
        .then(data => {
