  ...
  >
    <el-table-column type="expand" :label="maxLevel" width="100" :class-name="hasChild==='1'?'cell'+layer:'cell0'+layer" v-if="isSearch">
      <template slot-scope="scope">
        <expand-table
          v-if="scope.row.subList.length>0"
    ...
    <el-table-column label="功能名称" prop="name" class-name="first-name">
      <template slot-scope="scope">{{ scope.row.name }}</template>
    </el-table-column>
    <el-table-column label="关键字" prop="address">
      <template slot-scope="scope">{{scope.row.keys!==''?scope.row.keys:'--'}}</template>
    </el-table-column>
    <el-table-column label="启用" prop="">
      <template slot-scope="scope">{{scope.row.valiable==='1'?'√':'X'}}</template>
    </el-table-column>
    <el-table-column prop="tag" label="操作">
      <template slot-scope="scope">
        <el-button type="text" size="mini" @click.stop="handleAgent($event, scope.row)">添加功能</el-button>
