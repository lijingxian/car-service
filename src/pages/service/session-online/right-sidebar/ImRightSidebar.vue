  ...
  <div class="im-right-sidebar">
    ...
    <biz-popup-tabs :open="openProduct" @close="openProduct = false" v-model="productName">
      <el-tab-pane label="基本信息" name="product">
        <product-base-info :id="id" :baseInfo="baseInfo" :isEdit="isEdit" ref="productBaseInfo"></product-base-info>
      </el-tab-pane>
    </biz-popup-tabs>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters('SessionOnline', ['activatedRoom', 'tabType']),
    isCs() {
  ...
  created() {},
  components: { BizPopupTabs, PersonInfo, WorkSheet, RollOut, PortraitPreference, ConsumPreference, OrderBaseInfo, ProductInfo, ProductBaseInfo },
  methods: {
    showOrder(row) {
