<template>
  <biz-grid class="membership-mgmt__base-info" v-loading="loading">
    ...
    <el-form ref="form" size="small" label-width="110px" :model="formModel" :rules="rules" :disabled="!isEdit" ::inline-message="true">
      <div v-if="formModel.memberType==='1'">
        <el-form-item label="人像" v-if="!!formModel.photo">
          <img :src="formModel.photo" width="60px" />
        </el-form-item>
        <el-form-item label="企业名" prop="enterpriseName">
          <el-input v-model="formModel.enterpriseName"></el-input>
        </el-form-item>
        <el-form-item label="关联V服店铺" prop="enterprise">
          <el-input v-model="formModel.enterprise" disabled></el-input>
        </el-form-item>
        <el-form-item label="联系人" prop="contactPerson">
          <el-input v-model="formModel.contactPerson"></el-input>
        </el-form-item>
        <el-form-item label="电话" prop="contactMobile">
          <el-input v-model="formModel.contactMobile"></el-input>
        </el-form-item>
      </div>
      <div v-if="formModel.memberType==='0'">
        <el-form-item label="手机号" prop="mobile">
          <el-input v-model="formModel.mobile" type="phone" maxlength="11" placeholder="请输入手机号" @blur="mobileBlur"></el-input>
        </el-form-item>
        ...
        <el-form-item v-if="!formModel.id" label="密码" prop="password">
          <el-input v-model="formModel.password" type="text" placeholder="请输入6-16位密码"></el-input>
        </el-form-item>
        <!-- <el-form-item v-if="!itemId.id" label="确认密码" prop="rePassword" class="isRequired">
              <el-input v-model="formModel.rePassword" type="text" placeholder="请输入6-16位密码"></el-input>
        </el-form-item>-->
        ...
        <el-form-item label="挪车码" prop="moveCarCode">
          <el-input v-model="formModel.moveCarCode" disabled></el-input>
        </el-form-item>
      </div>
      <el-form-item label="专属顾问" prop="consultant">
        <el-select v-model="formModel.consultant.id" clearable placeholder="请选择专属顾问">
          <el-option v-for="(item,index) in consultant" :key="index" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="地址" v-if="formModel.memberType==='0'">
        <el-cascader :options="options" @change="handleChange" v-model="areas" style="width:100%"></el-cascader>
      </el-form-item>
      <el-form-item label="地址" v-if="formModel.memberType==='1'">
        <el-input v-model="formModel.address"></el-input>
      </el-form-item>
      <!-- <el-form-item label="邮编" prop="zipCode">
              <el-input v-model="formModel.zipCode" placeholder="请输入邮编"></el-input>
      </el-form-item>-->
      <el-form-item label="状态" prop="enableState">
        <el-switch v-model="formModel.enableState" active-value="1" inactive-color="#d9d9d9" active-color="#87d068" inactive-value="0"></el-switch>
      </el-form-item>
      <el-form-item label="用户来源" v-if="!!formModel.id">
        <el-select v-model="formModel.userSource" placeholder="请选择" style="width: 100%;" disabled>
          <el-option v-for="(item,index) in sourceList" :key="index" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="会员标签">
        <div style="display:flex;flex-flow: wrap;">
          <el-popover placement="bottom-end" width="200" trigger="hover" v-for="(item, index) in formModel.shopTags" :key="index">
            <div>
              {{item.hierarchy==='0'?'平台级':item.hierarchy==='1'?'店铺级':item.hierarchy==='2'?'运营商级':item.hierarchy==='3'?'模板级':'用户级'}},
              来源：{{item.tagCategory?item.tagCategory.fullName:''}}
              <span
                v-if="item.description"
              >,{{item.description}}</span>
            </div>
            <div class="tag-container" slot="reference">
              <el-tag :closable="item.deletable&&item.deletable==='0'?false:true" @close="onClose(item)" class="member-tag">{{item.name}}</el-tag>
            </div>
          </el-popover>
          <div>
      ...
      <el-form-item label="所属店铺">
        <el-table size="small" :data="formModel.shops">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button type="text" size="mini" @click="handleDeleteClick(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
      ...
      <el-form-item label="最后登录地址" prop="name" v-show="!!formModel.id" v-if="formModel.memberType==='0'">
        <span class="form-static">{{formModel.loginAddress||formModel.lastLoginIp}}</span>
      </el-form-item>
      <el-form-item label="最后登录日期" prop="name" v-show="!!formModel.id" v-if="formModel.memberType==='0'">
        <span class="form-static">{{formModel.lastLoginDate}}</span>
      </el-form-item>
      <el-dialog title="添加标签" :visible.sync="showSelecTags" width="600px" append-to-body class="member-ship-info">
        <div style="justify-content: space-between;display:flex">
          <el-cascader :props="{ checkStrictly: true }" :show-all-levels="true" :options="tagTypeOptions" v-model="tagClass" @change="tagTypeChange"></el-cascader>
          <biz-filter :filters="filters" @filterDataChange="filterDataChange" style="float:right"></biz-filter>
        </div>
        <div class="tag-list">
          <div class="recommend-group" v-if="tagOptions.length">
            <el-button size="small" v-if="tagClass[0]===''||tagClass[0]==='226'" @click="addTags">+自定义</el-button>
            <el-tag v-for="(tag, index) in tagOptions" :key="index" type="info" size="mini" :label="tag.id" @click="addNewTag(tag)">{{ tag.name }}</el-tag>
          </div>
          <div v-else class="empty">
            <el-button size="small" v-if="tagClass[0]===''||tagClass[0]==='226'" @click="addTags">+自定义</el-button>暂无标签数据
          </div>
          <!-- <el-checkbox-group v-model="selectTag" size="small" @change="tagChange">
            <el-button size="small" v-if="tagClass[0]===''||tagClass[0]==='226'" @click="addTags">+自定义</el-button>
            <el-checkbox-button v-for="(tag, index) in tagOptions" :label="tag.id" :key="index" class="tag-column">{{tag.name}}
            </el-checkbox-button>
          </el-checkbox-group>-->
        </div>
        <div class="tag-group">
          <el-tag
            ...
            :key="index"
            :closable="tag.deletable&&tag.deletable==='0'?false:true"
            @close="onCloseTag(tag, index)"
            class="sel-tag"
          >{{tag.name?tag.name:tag}}</el-tag>
        </div>
        <biz-save-button slot="footer" :operations="tagOperations" @operate="tagOperate"></biz-save-button>
      </el-dialog>
      <el-dialog title="添加自定义标签" :visible.sync="showNewTag" width="600px" append-to-body>
        <el-form ref="formTag" size="small" label-width="80px" :model="formTag" :rules="rulesTag">
          <el-form-item label="标签名称" prop="formTag">
            <el-input v-model="formTag.name" placeholder="请输入标签名称" style="width:210px"></el-input>
          </el-form-item>
          <el-form-item label="数据类型" prop="type">
            <el-select v-model="formTag.type" placeholder="默认无">
          ...
          <el-form-item label="标签说明" prop="description">
            <el-input v-model="formTag.description" placeholder="请输入该标签的意义，比如评定标准等" type="textarea" rows="4"></el-input>
          </el-form-item>
        </el-form>
        <biz-save-button :operations="operationsTag" @operate="operateHandlerTag"></biz-save-button>
      </el-dialog>
    </el-form>
  </biz-grid>
</template>
  ...
  methods: {
    ...
    getSaveParams() {
      if (this.areas && this.areas.length > 0) {
        this.formModel.area.id = this.areas[1];
      } else {
        this.formModel.area.id = '';
      }
      return {
        token: '',
