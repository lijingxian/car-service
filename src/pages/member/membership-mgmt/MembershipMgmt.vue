<template>
  <div class="membership-mgmt">
    ...
    >
      ...
      <el-table-column v-for="col of columns" :key="col.id" :label="col.name" :min-width="getWidth(col)" :sortable="getSortable(col)" :prop="getProp(col)">
        <template slot-scope="scope">
          ...
          <div v-else-if="col.id === '2'"><span :style="{color: stateColor(scope.row)}">{{scope.row.enableState==='1'?'启用':'停用'}}</span></div>
          <div v-else-if="col.id === '3'">
            <div class="customer-info" v-for="(shop,index) in scope.row.shops" :key="index">{{shop.name||'--'}}</div>
          </div>
          <div v-else-if="col.id === '4'">{{scope.row.consultant?(scope.row.consultant.name||'--'):'--'}}</div>
          <div v-else-if="col.id === '5'" class="customer-info">
            <span style="display:block">{{scope.row.loginAddress||scope.row.lastLoginIp||'--'}}</span>
            <span style="display:block">{{loginDateConverter(scope.row)}}</span>
          </div>
          <div v-else-if="col.id === '6'">
            <template v-if="!scope.row.shopTagsNew">-</template>
            <div class="platform-tags" v-for="(item, index) in scope.row.shopTagsNew" :key="index">{{item.name||'--'}}</div>
            <div class="show-more" v-if="scope.row.shopTags&&scope.row.shopTags.length>3">更多</div>
          </div>
        </template>
      </el-table-column>
    ...
    <div slot="float">
      ...
      <biz-popup-tabs :open="openPopup" v-model="popupTabName" @close="openPopup = false;">
        <el-tab-pane label="余额储值" name="storeinfo">
          <store-info ref="storeInfo" :itemId="chargeId" :userId="memberId">
          </store-info>
        </el-tab-pane>
      </biz-popup-tabs>
    </div>
    <el-dialog :visible.sync="dialogVisible" title="分配顾问">
      <el-select v-model="adminId" placeholder="请选择">
        <el-option v-for="item in consultantList" :key="item.value" :label="item.label" :value="item.value"></el-option>
      </el-select>
...
import StoreInfo from '@/pages/member/member-card-mgmt/MemberStoreInfo';
    ...
    getConsultant() {
      ...
      http
        ...
        .then(data => {
          ...
          this.consultantList = data.dataList.map(item => {
            return { label: item.name, value: item.id };
          });
        })
        .catch(errData => {
  ...
  methods: {
    ...
    confirm() {
      ...
      http
        ...
        .then(data => {
          if (JSON.stringify(data.unDistributed) === '{}') {
    ...
    cascaderChange(val) {
      ...
      if (val.length === 0) {
        ...
        if (val.length > 1 && !this.curTagName) {
          tagCategoryId = val[val.length - 2].id;
          http
...
const dataTableItem = {
  ...
  methods: {
    ...
    // 添加
    add() {
      this.$refs.popup.tabSelection = '0';
      this.open = true;
      this.openPopup = false;
      this.isEdit = true;
      this.itemId = { id: '' };
