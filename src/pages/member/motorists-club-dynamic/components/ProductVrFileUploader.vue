<template>
  <div class="product-vr-file-uploader">
    <el-upload accept="application/zip,video/*" name="uploadName" :action="uploadAction" :data="uploadData" :file-list="fileList"
      :on-remove="handleRemove" :on-success="handleSuccess" :on-error="handleError" :limit="1" :before-upload="beforeAvatarUpload" v-show="!disabled">
      <el-button size="small" type="primary">点击上传</el-button>
    </el-upload>
    <div class="view" :title="fileList[0]?fileList[0].url:''" v-show="disabled">{{fileList[0]?fileList[0].url:''}}</div>
    <el-button v-show="hasFile()" size="small" type="primary" @click.stop="handlePreview" :disabled="!preview" :loading="loading">预览</el-button>
...
export default {
  ...
  computed: {
    uploadAction() {
      if (!this.isTestDrive) {
        return '/admin/shopProduct/productVrZip.jhtml';
      } else {
