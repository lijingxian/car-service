<template>
  <biz-grid class="general-awards-setting">
    <biz-header slot="top" :operations="operations" @operate="operate">
    </biz-header>
    <el-collapse v-model="activeNames">
      <el-form ref="form" size="small" label-width="205px" :model="form" :rules="rules" style="border-bottom:1px solid #ebeef5">
...
export default {
  ...
  methods: {
    ...
    getList() {
      http
        .get('/admin/rewardSet/generalSetting.jhtml', {
    ...
    save() {
      this.$refs.form.validate(isValid => {
        if (isValid) {
          ...
          http
            .post('/admin/rewardSet/generalSetting.jhtml', this.form)
            .then(data => {
              this.$message.success('保存成功');
              this.$emit('closeSettingTab');
              this.loading = false;
            })
            .catch(ErrorData => {
