<template>
  <biz-grid class="agent-check-acount" v-loading="loading">
    <div slot="top">
    ...
    <biz-list ref="table" :tableData="tableData" @selection-change="handleSelectionChange" size="small" height="100%">
      ...
      <el-table-column label="代理商" min-width="100">
        <template slot-scope="scope">
          <div style="width:100%;color:#ff7300" @click.stop="agentClick(scope.row)">
            <div class="overflow">
    ...
    <biz-popup-tabs :open="openAgent" @close="openAgent = false;" v-model="tabSection">
      <el-tab-pane label="基本信息" name="baseinfo">
        <agent-base-info :itemId="agentId" :isEdit="isEdit" :parentId="parentId" :parentName="parentName" :grade="grade"></agent-base-info>
      </el-tab-pane>
    </biz-popup-tabs>
    <div slot="bottom">
...
export default {
      ...
      operations: [
        {
          ...
          name: 'exportStart',
          type: '',
          auth: ['admin:finance.reconciliation.agent.export']
        },
        {
          label: '刷新',
          name: 'refresh',
  ...
  methods: {
    ...
    agentClick(row) {
      this.agentId = { id: String(row.agent.id) };
      this.isEdit = false;
      this.openAgent = true;
    },
    exportStart() {
      ...
      http
        .post('/admin/purchaseOrderManage/agentBill/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/purchaseOrderManage/agentBill/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
    ...
    getList() {
      ...
      this.loading = true;
      http
        .get(urls.finance.supplierCheckAcount.agentList, params)
        .then(data => {
