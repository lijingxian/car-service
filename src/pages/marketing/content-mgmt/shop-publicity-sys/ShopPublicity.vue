...
export default {
  ...
  data() {
    return {
      ...
      operations: [
        ...
        { label: '批量审核通过', name: 'passCheck', type: 'primary', auth: ['admin:article.edit'], class: 'hidden' },
        { label: '设置', name: 'setting', type: 'primary', auth: ['admin:article.set'], class: 'hidden' }
      ],
      // '全部'选项
  ...
  methods: {
    ...
    del(id) {
      ...
      this.$confirm('是否确认删除?', '提示', {
        ...
        .then(() => {
          ...
          this.loading = true;
          http
            .delete('/admin/article/article/platformDelete.jhtml', params)
            .then(data => {
              this.loading = false;
    ...
    refresh() {
      ...
      this.loading = true;
      http
        .get('/admin/article/platformArticles.jhtml', params)
        .then(data => {
          this.loading = false;
