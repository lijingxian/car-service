<template>
 <div class="marketing-statistics-content-kpi-report">
...
export default {
    ...
    dataFormat(data) {
      ...
      for (let i = 0; i < data.histogram.length; i++) {
        tmpData.push({
          ...
          [this.kpiName[1]]: data.lineChat.length === 0 ? '' : ((data.lineChat[i].yValue === '0' || data.lineChat[0].yValue === undefined) ? '0' : data.lineChat[i].yValue / 100)
        });
      }
      let chartData = {
        type: 'histogram',
  ...
  computed: {
    kpiName: function() {
      if (this.filters[1].value === 'read') {
        return ['阅读数', '阅读率'];
      }
      if (this.filters[1].value === 'attend') {
        return ['关注数', '增长率'];
      }
      return [];
    }
...
<style lang="scss">
.marketing-statistics-content-kpi-report {
  height: 100%;
  overflow: auto;
