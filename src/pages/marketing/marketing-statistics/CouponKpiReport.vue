<template>
 <div class="marketing-statistics-coupon-kpi-report">
...
export default {
    ...
    return {
      ...
      filters: [
        ...
        },
        {
          ...
          value: 'total',
          options: [
            { value: 'total', label: '优惠券发放量' },
            { value: 'received', label: '优惠券领取量' },
            { value: 'used', label: '优惠券使用量' }
          ]
        },
        {
          type: 'daterange',
  ...
  computed: {
    kpiName: function() {
      ...
      if (this.filters[1].value === 'received') {
        return ['优惠券领取量', '领取率'];
      }
      if (this.filters[1].value === 'used') {
        return ['优惠券使用量', '使用率'];
      }
      return [];
    }
...
<style lang="scss">
.marketing-statistics-coupon-kpi-report {
  height: 100%;
  overflow: auto;
