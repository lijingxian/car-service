<template>
  <div class="rank-item">
    <el-radio-group v-model="rankData.publishTarget" @change="onTypeChange">
      <el-radio label="0">所有会员</el-radio>
      <el-radio label="1">指定等级会员</el-radio>
      <el-radio label="2">指定会员</el-radio>
    </el-radio-group>
    <div v-if="rankData.publishTarget==='0'">
      <span>已选中会员{{total}}人</span>
    </div>
    <div v-else-if="rankData.publishTarget==='1'">
    ...
    <biz-dialog-selector title="选择会员" :visibility.sync="showDialog" :appendToBody="true" :keyField="'id'" :selectionKeys="selectionKeys"
      :loading="dialogLoading" :tableData="dialogTableDataHandle" :pagination="dialogPagination" :filters="dialogFilters"
      @confirmed="handleConfirmed" @filterDataChange="dialogFilterDataChange" @pageSizeChange="dialogPageSizeChange"
      @currentPageChange="dialogCurrentPageChange" @selectionChange="selectionChange" class="rank-dialog-selector"
...
export default {
  ...
  computed: {
    ...
    vipRankList() {
      console.log(123);
      let ary = [];
      this.rankData.memberRankList.forEach(item => {
        let obj = {};
        obj.id = item.id;
        obj.name = item.name;
        obj.ranks = [];
      ...
      for (const memberRank of memberRankList) {
        for (const vip of memberRank.ranks) {
          for (const iterator of this.rankData.memberRanks) {
  ...
  methods: {
    ...
    clearData() {
      let memberRankList;
      switch (this.rankData.publishTarget) {
        case '0':
          memberRankList = this.rankData.memberRankList;
          for (const memberRank of memberRankList) {
            for (const vip of memberRank.ranks) {
              vip.checked = false;
            }
    ...
    getVipCount(ary) {
      ...
      } else if (this.rankData.publishTarget === '1') {
        if (!ary || !ary.length) {
          return;
        }
    ...
    handleConfirmed(selectionKeys, selections) {
      ...
      if (this.rankData.members.length) {
        list.forEach(item => {
    ...
    memberRankHandle() {
      let ary;
      this.selVipList();
      ary = this.rankData.memberRanks.map(item => {
    ...
    selVipList() {
      let curSel;
      this.selVips = [];
      console.log('111', this.vipRankList);
    ...
    vipChange(val) {
      if (val.checked) {
        this.rankData.memberRanks.forEach((element, index) => {
          if (element.id === val.id) {
            this.rankData.memberRanks.splice(index, 1);
          }
        });
      } else {
        this.rankData.memberRanks.push({ id: val.id });
      }
    },
    getTagClassList() {
      let param = {
