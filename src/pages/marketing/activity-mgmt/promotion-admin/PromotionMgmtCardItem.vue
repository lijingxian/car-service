  ...
  <div class="promotion-mgmt-card-item" @click="select">
    ...
    <div class="detail">
      ...
      <div class="sendButton">
        <el-button type="primary" style="padding: 7px 15px;" @click.stop="handleSend(promotion)">发布</el-button>
        <el-button type style="padding: 7px 15px;" @click.stop="handleDelete(promotion)">删除</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSend(row) {
      ...
      this.loading = true;
      http
        .post('/admin/promotion//platform/publish.jhtml', params)
        .then(data => {
          this.$message({
            type: 'success',
            message: '发布成功!'
      ...
      if (row.isSubmit === '1') {
        this.$confirm('是否确认删除?', '提示', {
          ...
          .then(() => {
            ...
            PromotionMgmtService.deletePromotionListAdmin(
              params,
              data => {
                this.$message({
                  type: 'success',
