      ...
      <el-table-column prop="date" label="操作" min-width="100">
        <template #default="scope">
          <biz-column-operation :row="scope.row">
            <el-button type="text" size="mini" @click.stop="Publish(scope.row)" v-if="canPublished(scope.row)">发布</el-button>
            <el-button type="text" size="mini" @click.stop="remove(scope.row)">删除</el-button>
    ...
    Submit(row) {
      this.$confirm('是否确认提交审核?', '提示', {
        ...
        .then(() => {
          ...
          activityMgmtService.submitAdmin(
            params,
            data => {
              this.$message({
                type: 'success',
