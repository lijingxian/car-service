<template>
  <biz-grid class="market-activity-activity-notice">
    ...
    <el-form ref="form" :model="notice" :rules="rules" size="small" label-width="110px" v-loading="loading" :disabled="!isEdit">
      ...
      <el-form-item label="通知方式">
        <div class="notify-type">
          ...
          <div class="item">
            <el-checkbox v-model="notice.isAppNotice">App消息</el-checkbox>
          ...
          <div class="item">
            ...
            <div class="preview-content" v-show="notice.isWechatNotice">
              ...
              <div>活动主体：
                <span class="custom-c">{{activity.title}}</span>
              </div>
              <div>通知时间：
                <span class="custom-c">{{activity.beginDate}}</span>
                -
                <span class="custom-c">{{activity.endDate}}</span>
              </div>
    ...
    <biz-dialog-selector title="选择会员" :visibility.sync="showDialog" :appendToBody="true" :keyField="'id'" :selectionKeys="selectionKeys"
      ...
      @filterDataChange="dialogFilterDataChange" class="notice-dialog-selector" @pageSizeChange="dialogPageSizeChange"
      @currentPageChange="dialogCurrentPageChange" @selectionChange="selectionChange" @cascaderChange="cascaderChange">
...
export default {
  ...
  computed: {
    ...
    app() {
      return this.defaultShop.name + '发布了' + (this.activity.title || '') + ',快去享受优惠吧！';
    },
    sms() {
      return (
        '【V服平台】亲爱的会员，为感恩长期以来大家对' +
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate(valid => {
        ...
        if (this.notice.noticeTarget === 'ToMemberrank' && this.rankTotal === 0) {
          this.$message.warning('请选择会员');
          return;
        }
        if (this.notice.noticeTarget === 'ToMember' && this.notice.noticeMembers.length === 0) {
          this.$message.warning('请选择会员');
          return;
        }
        this.loading = true;
        ...
        this.loading = true;
        Service.updateLocalActivityNotice(
          params,
          data => {
    ...
    publish() {
      ...
      Service.publishLocalActivityNotice(
        {
          activityId: this.activity.id
        },
        data => {
    ...
    refresh(activity) {
      ...
      Service.getLocalActivityNotice(
        params,
        data => {
          if (JSON.stringify(data.data) === '{}') {
            this.clear();
            this.notice.content = this.defaultShop.name + '发布了' + (this.activity.title || '') + ',快去享受优惠吧！';
