...
let containerHeight = window.innerHeight - 190;
...
let tableView = function() {
  transform(targets.table, 2000);
};
let sphereView = function() {
  transform(targets.sphere, 2000);
};
let helixView = function() {
  transform(targets.helix, 2000);
};
      ...
      http
        ...
        .then(data => {
          ...
          if (data.content.length < 100) {
            for (let i = 0; i < 150 - data.content.length; i++) {
              data.content.push({ member: { head: '/resources/admin/images/Avatar.png', userName: '' } });
            }
          }
          for (let i = 0; i < data.content.length; i++) {
            let item = {};
            item.name = data.content[i].member.userName;
            item.head = data.content[i].member.head || '/resources/admin/images/Avatar.png';
            item.p_x = (i % 18) + 1;
