<template>
  <div class="promotion-participation-way">
    ...
    <biz-dialog-selector title="选择商品" :visibility.sync="showProductSelector" :tableData="productTableData"
      ...
      <el-table-column property="categoryType" label="分类">
        <template slot-scope="scope">
          {{scope.row.categoryType === '0' ? '整车' : (scope.row.categoryType === '1' ? '车品' : (scope.row.categoryType === '2' ?
          '服务' : '无'))}}
        </template>
      </el-table-column>
    </biz-dialog-selector>
...
import service from './ParticipationWay.js';
  ...
  methods: {
    ...
    getProductTable() {
      ...
      service.queryProductList(
        ...
        ErrorData => {
          console.log('ERR_GET_PRODUCT_TABLE: ', ErrorData);
        }
      );
    },
    addProducts() {
