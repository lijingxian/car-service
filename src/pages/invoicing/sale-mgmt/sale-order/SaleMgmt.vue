<template>
  <div class="order-mgmt-list-p">
    ...
    <biz-flex-table ref="table" :operations="operations" @operate="operate" :filters="filters"
      ...
      @selection-change="handleSelectionChange" @select="select" @row-click="rowClick" @size-change="handleSizeChange"
      @current-change="handleCurrentChange" class="order-mgmt-tabel" :isMergeInvoice="true" :showSummary="true" @closeDialogFilter = "closeDialogFilter" >
      ...
      <el-table-column label="代理商" min-width="120px">
        <template slot-scope="scope">
          <div style="width:100%">
            <div class="overflow" style="color:#ff7300" @click.stop="agentClick(scope.row)">
      ...
      <el-table-column label="关联单据" min-width="170">
        <template slot-scope="scope">
          <div style="width:100%">
            <div v-for="(relateBill,index) in scope.row.relateBills" :key="index">
              <div style="color:#ff7300;cursor:pointer" class="shop-over" :title="relateBill.relateBillSn ? relateBill.relateBillSn : ''" @click="clickRelateBill($event, relateBill)" v-if="relateBill.relateBillSn">
      ...
      <el-table-column prop="date" label="操作" min-width="90">
        <template #default="scope">
          <biz-column-operation :row="scope.row"
            v-if="(scope.row.unionConfig.editable === '1' || getUser().roleLevel !== 'shop') && scope.row.isValid === '1'">
            <el-button type="text" size="mini" v-if="scope.row.type==='9' && scope.row.hasSyncOrder === '0'"
              @click.stop="refundsHandler(scope.row)">退款</el-button>
            <el-button type="text" size="mini"
              v-if="scope.row.type==='2'&&(orderType==='ride'||orderType==='replace'||orderType==='loan') && scope.row.hasSyncOrder === '0'"
              @click.stop="service(scope.row)">服务</el-button>
            <el-button type="text" size="mini"
              ...
              @click.stop="serviceHandler(scope.row)">服务</el-button>
            <el-button type="text" size="mini"
              ...
              @click.stop="excuteHandler(scope.row)">服务</el-button>
            <el-button type="text" size="mini"
              v-if="(orderType==='service'||orderType==='rescue')&&scope.row.type==='3'&&scope.row.shippingWay==='0' && scope.row.hasSyncOrder === '0'"
              @click.stop="excuteHandler(scope.row)">服务</el-button>
            <el-button type="text" size="mini"
              v-if="((scope.row.type==='13')||(scope.row.type==='3'&&scope.row.shippingWay==='0')||(scope.row.type==='2'&&scope.row.shippingWay==='1'))&&(orderType==='car'||orderType==='product'||orderType==='general'||scope.row.dType==='purchaseOrder') && scope.row.hasSyncOrder === '0' && orderType !== 'service'"
              @click.stop="shippingHandler(scope.row)">发货</el-button>
            <el-button type="text" size="mini"
              v-if="((scope.row.type==='11'&&scope.row.shippingWay==='1')||(scope.row.type==='12'&&scope.row.shippingWay==='1')) && scope.row.hasSyncOrder === '0'"
              @click.stop="paymentHandler(scope.row)">付款</el-button>
            <el-button type="text" size="mini" v-if="scope.row.type==='2'||scope.row.type==='1'"
              @click.stop="closeOrder(scope.row)">关闭</el-button>
            <el-button type="text" size="mini" @click.stop="certificate(scope.row)">凭证</el-button>
            <el-button type="text" size="mini" v-if="scope.row.type!=='6'"
              @click.stop="invoiceHandler(scope.row)">申请发票</el-button>
            <el-button type="text" size="mini"
              v-if="scope.row.type==='1'&&((scope.row.auditState==='1'&& scope.row.audit==='1') || (scope.row.auditState==='2'&& scope.row.audit==='1'))"
              @click.stop="toPass(scope.row)">审核</el-button>
            <el-button type="text" size="mini" v-if="scope.row.isMerged==='0' && scope.row.type==='2'"
              @click.stop="mergeOrder(scope.row)">合并订单</el-button>
            <el-button type="text" size="mini"
              v-if="scope.row.isTransfered==='0'&&(scope.row.type==='2'||scope.row.type==='3'||scope.row.type==='4'||scope.row.type==='12'||scope.row.type==='13')"
              @click.stop="returnOrder(scope.row)">转采购</el-button>
    ...
    <biz-popup-tabs :open="openSettingPrice" @close="openSettingPrice=false" v-model="tabName">
      <el-tab-pane label="设置" name="settinginfo">
        <price-setting ref="settingPrice" @operationSuccess="operationSuccess"></price-setting>
      </el-tab-pane>
    </biz-popup-tabs>
    ...
    <biz-popup-tabs :open="openLogistics" @close="openLogistics=false" v-model="tabName">
      <el-tab-pane label="物流追踪" name="loginfoTab">
    ...
    <biz-popup-tabs :open="open" @close="open=false" v-model="tabName">
      <el-tab-pane label="基本信息" name="info">
        <base-info :itemId="itemId" :isEdit="isEdit" ref="baseInfo" :handler="handler" :open="open" :activeNames="activeNames"
          @xrorder="xrorder" @operationSuccess="operationSuccess"></base-info>
      </el-tab-pane>
      <el-tab-pane label="商品信息" name="productinfo">
        <product-info :itemId="itemId" :isEdit="isEdit" :agentId="agentId" :open="open" ref="productinfo" @setOrderItems="setOrderItems"
          @operationSuccess="operationSuccess"></product-info>
      </el-tab-pane>
      <el-tab-pane label="订单进度" name="loginfo" v-if="itemId">
        <order-logs :itemId="itemId" :isEdit="isEdit" ref="loginfo" :paymentMethodName="paymentMethodName" @logistics="logistics">
        </order-logs>
      </el-tab-pane>
      <el-tab-pane label="物流信息" name="logisticsinfo" v-if="itemId">
        <logistics-info :itemId="itemId" ref="logisticsinfo" :open="open" :handler="handler" :shippings="shippings"
          @operationSuccess="getList" @goToOrderLogs="goToOrderLogs" :itemList="itemList"></logistics-info>
      </el-tab-pane>
      ...
      <el-tab-pane label="发票" name="invoice" v-if="itemId&&orderState!=='6'">
        <invoice-info :itemId="itemId" :invoiceInfoId="invoiceInfoId" :invoiceId="invoiceId" :price="price" :open="open"
          :userId="userId" :isEdit="isEdit" ref="invoice" @operationSuccess="getList" :category="'1'" :buyerType="'2'" :buyer="buyer" :sellerType="'5'" :seller="getUser().shop.id"></invoice-info>
      </el-tab-pane>
      <el-tab-pane label="子订单信息" name="orderChild" v-if="mergeState==='1'">
        <order-child-info :itemId="itemId" :invoiceInfoId="invoiceInfoId" :invoiceId="invoiceId" :price="price" :open="open"
          :userId="userId" :isEdit="isEdit" ref="orderChild" @operationSuccess="operationSuccess"></order-child-info>
      </el-tab-pane>
    </biz-popup-tabs>
    <biz-popup-tabs :open="openInvocie" @close="openInvocie=false" v-model="tabName">
      <el-tab-pane label="发票" name="invoice">
        <invoice-info :itemId="itemId" :invoiceInfoId="invoiceInfoId" :invoiceId="invoiceId" :price="price" :open="openInvocie"
          :userId="userId" :isEdit="isEdit" :orderIds="orderIds" ref="invoice" @operationSuccess="getList" :category="'1'" :buyerType="'2'" :buyer="buyer" :sellerType="'5'" :seller="getUser().shop.id"></invoice-info>
      </el-tab-pane>
    </biz-popup-tabs>
    ...
    <biz-popup-tabs :open="openPurchase" @close="openPurchase = false" v-model="tabName">
      <el-tab-pane label="基本信息" name="info">
        <purchase-apply-info :itemId="purchaseId" :isEdit="isEdit" @operationSuccess="operationSuccess"
          @closeTabByLogistics="closeTabByLogistics" @closeTabByEvaluate="closeTabByEvaluate"></purchase-apply-info>
      </el-tab-pane>
    </biz-popup-tabs>
    <biz-popup-tabs :open="openPurchaseOrder" @close="openPurchaseOrder=false" v-model="tabName">
      <el-tab-pane label="基本信息" name="info">
        <purchase-order-info :orderId="purchaseOrderId" ref="orderinfo" :isEdit="isEdit" :isRalateBill="isRalateBill"
          @closeTabByLogistics="closeTabByLogistics" @closeTabByEvaluate="closeTabByEvaluate">
        </purchase-order-info>
      </el-tab-pane>
    </biz-popup-tabs>
    <biz-popup-tabs :open="openOrderSale" @close="openOrderSale = false" v-model="tabName">
      ...
      <el-tab-pane label="商品信息" name="OrderSaleProductInfo">
        <order-sale-product-info ref="OrderSaleProductInfo" :itemId="orderSaleId" :isEdit="isEdit" @operationSuccess="refresh">
        </order-sale-product-info>
      </el-tab-pane>
    </biz-popup-tabs>
    <biz-popup-tabs :open="openAgent" @close="openAgent = false;" v-model="tabSection">
      <el-tab-pane label="基本信息" name="baseinfo">
        <agent-base-info :itemId="agentTabId" :isEdit="isEdit" :parentId="parentId" :parentName="parentName" :grade="grade"></agent-base-info>
      </el-tab-pane>
      <el-tab-pane label="代理商品" name="productinfo">
        <agent-product-info :itemId="agentTabId" :isEdit="isEdit" :grade="grade" ref="agnetProduct"></agent-product-info>
      </el-tab-pane>
      <el-tab-pane label="销售记录" name="salesinfo">
        <sales-info :itemId="agentTabId" :isFrom="true"></sales-info>
      </el-tab-pane>
    </biz-popup-tabs>
    <el-dialog :visible.sync="showPassOrder" center append-to-body custom-class="close-order-dialog" title="审核"
      v-loading="loading">
      <div>
        <div class="reson-container">
          ...
          <div style="padding: 5px 0px;margin-top:15px;" v-show="auditState==='2'">
            ...
            <el-select v-model="curSelectAjectReason" placeholder="请选择驳回理由" class="order-select" size="mini">
              <el-option v-for="reason in reasonAjectList" :key="reason.label" :label="reason.label" :value="reason.value">
              </el-option>
            </el-select>
          </div>
        ...
        <div class="d-p-wrap">
          ...
          <el-table :data="gridData" class="table-class">
            ...
            <el-table-column property="barCode" label="条码">
              <template
                slot-scope="scope">{{scope.row.product&&scope.row.product.barCode?scope.row.product.barCode:''}}</template>
            </el-table-column>
    ...
    <biz-dialog-selector title="合并订单" :visibility.sync="showMergeOrder" :tableData="orderList" :pagination="dialogPagination"
      :selectionKeys="selectedOrderIds" @pageSizeChange="dialogPageSizeChange" @currentPageChange="dialogCurrentPageChange"
      class="product-info-dailog" @confirmed="confirmedMergeOrder" @selectionChange="selectionChange">
            ...
            <div style="width:100%;margin-right:30px">
              <div v-for="orderItem in scope.row.orderItems" :key="orderItem.id" class="product-info">
                ...
                <div class="info-bottom">
                  <div style="margin-left:7px;opacity:0.65">
                    {{orderItem.fullName&&orderItem.fullName.indexOf('[')!==-1?orderItem.fullName.split("[")[1].split("]")[0]:''}}
                  </div>
                  <div></div>
                </div>
              </div>
      ...
      <el-table-column label="创建人" min-width="90px">
        <template slot-scope="scope">{{scope.row.operatorName}}</template>
      </el-table-column>
    </biz-dialog-selector>
    <el-dialog :visible.sync="showOrderContactor" center append-to-body custom-class="order-contactor-dialog" title="合并订单"
      v-loading="loading">
      <div>
    ...
    <el-dialog :visible.sync="showSupplier" append-to-body title="提交采购订单">
      <div style="margin:-20px 0px 10px 0px">请选择需要采购的商品和供应商,填写商品的采购数量</div>
      <el-table @selection-change="selectionChangeMethod" :data="tableDataContent" size="small">
        <el-table-column type="selection" width="55"></el-table-column>
        <el-table-column label="商品" min-width="120">
          <template slot-scope="scope">
            <div>{{ scope.row.product.fullName }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="供应商" min-width="120">
          <template slot-scope="scope">
            <el-select v-model="scope.row.supplierId" size="mini">
              <el-option v-for="purchasePrices in scope.row.product.purchasePrices" :key="purchasePrices.cooperationId" :value="purchasePrices.cooperationId" :label="purchasePrices.cooperationName"></el-option>
            </el-select>
          </template>
        </el-table-column>
        <el-table-column label="数量" min-width="120">
          <template slot-scope="scope">
            <el-input-number :min="0" v-model="scope.row.quantity" size="mini"></el-input-number>
          </template>
        </el-table-column>
      </el-table>
      <biz-save-button :operations="formOperations" @operate="operate" style="margin-top: 10px;">
      </biz-save-button>
    </el-dialog>
    <el-dialog :visible.sync="showReturnPurchase" center append-to-body custom-class="return-purchase-dialog" title="提交采购订单">
      <div v-if="productGroupWithNoSupplier.length>0">
              ...
              <div v-for="(item,index) of noSupplier.cartItems" :key="index">
                <div class="order-goods-div">
                  ...
                  <div class="goods-info">
                    ...
                    <div style="margin-bottom:10px;">{{item.product.barCode}}</div>
          ...
          <div v-if="productGroupNoSupplierPrice.length>0">
            <div>以下商品无采购价，暂不支持提交采购订单</div>
            <div v-for="(noPrice,index) of productGroupNoSupplierPrice" :key="index">
              <div v-for="(item,index) of noPrice.cartItems" :key="index">
                <div class="order-goods-div">
                  <div class="img-div">
                    <img v-if="item.product.image&&item.product.image!==' '" :src="item.product.image" :onerror="errorImg" />
                  </div>
      ...
      <div class="order-confirm" v-loading="loadingReturnPurchase">
        <div class="order-confirm-content">
          <div v-if="returnOrderList.length>0" style="position: relative;">
            <div
              style="display: flex;align-items: center;position: absolute;z-index: 1000;left: 18px;top: -2px;background: #fff;width:80%">
              <img :src="iconShop" style="width:18px;height:18px" />
              {{agentAddress?agentAddress.shop.name:''}}
            </div>
            <div
              style="display:flex;align-items: center;position: absolute;z-index: 1000;left: 18px;top: 95px;background: #fff;width:80%">
              <img :src="iconShop" style="width:18px;height:18px" />
              {{agentAddress?agentAddress.agent.name:''}}
            </div>
            <el-radio-group v-model="selectRadio" style="width: 100%;" @change="radioChange">
              <el-radio label="shopReceiver" style="display:flex;align-items: center;">
              </el-radio>
                  ...
                  <div class="address">
                    ...
                    <div class="icon">
                      <span>
                        ...
                        {{(getAddress&&getAddress.areaName&&getAddress.address)?(getAddress.areaName+getAddress.address):'请设置收货地址'}}
                      </span>
                      <span>
              ...
              <el-radio label="agencyReceiver" style="display:flex;align-items: center;">
              </el-radio>
              <div class="order-address">
                <div class="left">
                  <div class="address">
                    <div class="receiver-address">
                      <span>{{agentAddress?agentAddress.consignee:''}}</span>
                      <span>{{agentAddress?agentAddress.phone:''}}</span>
                    </div>
                    <div class="icon">
                      <span>
                        <i class="el-icon-location-outline"></i>
            ...
            <div v-for="(returnList,index) in returnOrderList" :key="index"
              style="box-shadow: 0 2px 3px rgba(0, 0, 0, 0.1);padding: 20px;margin-bottom: 10px;">
              <div class="order-goods">
                ...
                <div v-for="(item,index) of returnList.cartItems" :key="index">
                  <el-checkbox v-model="item.isSelected" :min="1" true-label="1" false-label="0" size="mini"
                    @change="selectReturnOrder(returnList)">
                    <span style="color:#ffffff">.</span>
                  </el-checkbox>
                  <div class="order-goods-div">
                    <div class="img-div">
                      <img v-if="item.product.image&&item.product.image!==' '" :src="item.product.image||'/resources/admin/images/default/product-default-100.png'" :onerror="errorImg" />
                    </div>
                    <div class="goods-info">
                      <span class="name">{{item.product.fullName}}</span>
                      <div style="margin-bottom:10px;">{{item.product.barCode}}</div>
                      <div class="money-info">
                        <span class="count-money">￥{{item.product.purchasePrices[0].price}}</span>
                        <el-input-number v-model.number="item.quantity" size="mini" class="count-num">X{{item.quantity}}
                        </el-input-number>
                      </div>
                    </div>
                  </div>
              ...
              <div class="order-info">
                <div class="select-tyle">
                  <span>支付方式</span>
                  <el-select v-model="returnList.paymentMethodId" @change="paymentChange(returnList.paymentMethodId,index)"
                    size="mini">
                    <el-option v-for="payOption in payOptions" :key="payOption.id" :label="payOption.name" :value="payOption.id">
                    </el-option>
                  </el-select>
                </div>
                <div class="select-tyle">
                  <span>配送方式</span>
                  <el-select v-model="returnList.shippingMethodId" @change="inputChange(returnList.shippingMethodId,index)"
                    size="mini">
                    <el-option v-for="optionItem in options" :key="optionItem.id" :label="optionItem.name" :value="optionItem.id">
                    </el-option>
                  </el-select>
                </div>
            ...
            <div class="order-info">
              <div class="order-dispatch">
                ...
                <div class="invoice-info" @click="clickInvoiceInfo">
                  <span class="invoice-title" style="font-size:14px"
                    v-if="invoiceChecked">{{getInvoice?getInvoice.invoiceTitle:'暂无发票'}}</span>
          ...
          <div class="foot" slot="bottom">
            <el-button class="submit" type="primary" @click="submit" v-show="returnOrderList.length>0">提交订单</el-button>
          </div>
        </div>
      </div>
    </el-dialog>
    ...
    <el-dialog :visible.sync="showApplyInvoice" center append-to-body custom-class="apply-invoice-dialog" width="1000px">
      <apply-invoice :invoiceAmount="invoiceAmount" :orderIds="orderIds" ref="applyInvoice" :userId="userId" :category="'1'" :buyerType="'2'" :buyer="buyer" :sellerType="'5'" :seller="getUser().shop.id"  @close="showApplyInvoice=false"></apply-invoice>
    </el-dialog>
...
import LogisticsInfo from './OrderMgmt/LogisticsInfo';
...
import LogisticsFollowing from '@/pages/invoicing/sale-mgmt/sale-order/OrderMgmt/LogisticsFollowing';
import AgentBaseInfo from '@/pages/invoicing/base-data/agent/AgentBaseInfo';
import AgentProductInfo from '@/pages/invoicing/base-data/agent/AgentProductInfo';
import SalesInfo from '@/pages/invoicing/base-data/agent/SalesInfo';
...
export default {
  ...
  computed: {
    ...
    hasAuthEdit() {
      // return true;
      return this.currentAuth.includes('admin:saleOrder.edit');
    },
    // 是否显示发票列
    toggleRecommendTxt() {
      ...
      for (const item of this.returnOrderList) {
        ...
        for (let i = 0; i < len; i++) {
          if (ary[i].isSelected === '1') {
            count += ary[i].product.purchasePrices[0].price * ary[i].quantity;
          }
        }
      }
  ...
  methods: {
    ...
    agentClick(row) {
      this.agentTabId = { id: String(row.agent.id) };
      this.tabSection = 'baseinfo';
      this.isEdit = false;
      this.openAgent = true;
    },
    settingPrice() {
      this.tabName = 'settinginfo';
      this.openSettingPrice = true;
    },
    ...
    confirmExport() {
      ...
      http
        .post('/admin/purchaseOrderManage/storeSalesOrders/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/purchaseOrderManage/storeSalesOrders/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
        console.log('response => ', response);
        const blob = new Blob([response.data], { type: 'application/vnd.ms-excel' });
    ...
    rowClick(model, e, column) {
      ...
      for (const item of model.shippings) {
        this.itemList.push(item.shippingItems);
      }
    ...
    shippingHandler(row) {
      ...
      this.shippings = row.shippings;
      for (const item of row.shippings) {
        this.itemList.push(item.shippingItems);
      }
      if ((row.unionConfig.editable === '0' && this.getUser().roleLevel === 'shop') || row.isValid === '0') {
        this.isEdit = false;
    ...
    mergeInvioce() {
      ...
      this.shippings = this.selections[0].shippings;
      this.tabName = 'invoice';
      this.orderIds = [];
      this.price = 0;
      this.selections.forEach(element => {
    ...
    radioChange(val) {
      if (val === 'agencyReceiver') {
        this.receiver = {
          consignee: this.agentAddress.consignee,
          areaName: this.agentAddress.areaName,
          address: this.agentAddress.address,
          zipCode: this.agentAddress.zipCode,
          phone: this.agentAddress.phone,
          shop: { name: this.agentAddress.operatorName }
        };
      } else {
        this.receiver = {};
      }
    ...
    getReturnPurchase() {
      ...
      http
        ...
        .then(data => {
          ...
          this.productGroupNoSupplierPrice = data.productGroupNoSupplierPrice;
          ...
          this.returnOrderList.map((item, index) => {
            this.tableDataContent = this.tableDataContent.concat(item.cartItems);
          });
    ...
    saveForm() {
      if (this.selectionsVal.length === 0) {
    ...
    selectionChangeMethod(val) {
      this.selectionsVal = val;
    ...
    returnOrderConfirm() {
      ...
      interfaceName = '/admin/purchaseOrder/saleToPurchase/fillIn.jhtml';
      let param = {
        cartItems: this.selectionsVal,
      ...
      http
        ...
        .then(data => {
          ...
          this.payOptions = data.productGroup[0].paymentMethods;
          this.options = data.productGroup[0].shippingMethods;

          this.payType = data.productGroup[0].paymentMethods[0].name;
          this.type = data.productGroup[0].shippingMethods[0].name;
          for (const item of this.selectionsVal) {
            item.paymentMethodId = data.productGroup[0].paymentMethods[0].id;
            item.shippingMethodId = data.productGroup[0].shippingMethods[0].id;
            if (data.productGroup[0].shippingMethods && data.productGroup[0].shippingMethods.length) {
              if (data.productGroup[0].shippingMethods[0].isNeedReceiver === '0') {
                this.showAddress = false;
              } else {
                this.showAddress = true;
              }
    ...
    submit() {
      ...
      let flag = false;
      for (const item of this.returnOrderList) {
        for (let i = item.cartItems.length - 1; i >= 0; i--) {
          if (!Number.isInteger(item.cartItems[i].quantity)) {
            this.$message.warning('数量不正确');
            return;
          }
          if (item.cartItems[i].isSelected === '1') {
            flag = true;
          }
        }
      }
    ...
    confirmReturnOrder() {
      this.selectionProduct = [];
      if (this.selectRadio === 'shopReceiver') {
        for (const item of this.returnOrderList) {
      ...
      this.loadingReturnPurchase = true;
      let params = {
        isAgentAddress: this.selectRadio === 'shopReceiver' ? '0' : '1',
      ...
      http
        ...
        .then(data => {
          ...
          this.loadingReturnPurchase = false;
          this.refresh();
        })
        .catch(ErrorData => {
          this.$message.warning(ErrorData.errorMessage);
