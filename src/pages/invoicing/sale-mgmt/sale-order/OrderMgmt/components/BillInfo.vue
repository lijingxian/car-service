<template>
  <biz-grid class="bill-info">
      ...
      <el-card v-for="receiver in receiverList" :key="receiver.id">
        ...
        <div class="bottom">
          <div v-if="receiver.isDefault==='1'">默认</div>
          <el-checkbox v-model="receiver.isDefault" v-if="receiver.isDefault==='0'" true-label="1" false-label="0" size="mini"
            @change="setDefault(receiver)">设为默认</el-checkbox>
          <div class="handler">
...
export default {
  ...
  data() {
    return {
      ...
      formOperations: [
        ...
        { label: '取消', name: 'cancel', type: 'primary' }
      ],
      rules: {
        consignee: [{ required: true, message: '请输入收货人姓名', trigger: ['blur', 'change'] }],
  ...
  methods: {
    ...
    selectItem(receiver) {
      this.$emit('selectedItem', receiver);
    },
