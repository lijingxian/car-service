<template>
  <biz-grid class="order-product-info">
    ...
    <el-form :inline="true" :disabled="!isEdit" :inline-message="true">
      ...
      <el-form-item label="合计:" prop="subTotal" class="subTotal">{{subTotal}}</el-form-item>
    </el-form>
    <biz-list ref="table" :tableData="tableData" :loading="loading" :key="Math.random()" style="width: 100%;height:100%">
      ...
      <el-table-column prop="price" label="商品价格">
         <div slot-scope="scope">
          <div class="biz-customcol">{{(scope.row.price*1).toFixed(4)}}
          </div>
        </div>
      </el-table-column>
      <el-table-column prop="minimumPackQuantity" label="最小包装数" min-width="60">
        <template
          slot-scope="scope">{{scope.row.product&&scope.row.product.minimumPackQuantity?scope.row.product.minimumPackQuantity:''}}</template>
      </el-table-column>
      ...
      <el-table-column prop="subTotal" label="小计">
        <div slot-scope="scope">
          <div class="biz-customcol">{{scope.row.quantity?(Number(scope.row.quantity)*Number(scope.row.price)).toFixed(2):''}}
          </div>
        </div>
      </el-table-column>
      ...
      <vue-easy-print tableShow ref="easyPrint">
        <div class="d-p-wrap">
          ...
          <el-row :gutter="10" class="row-top">
            <el-col :span="6">
              <div class="grid-content bg-purple">订单编号：{{shippingData.sn}}</div>
            </el-col>
            <el-col :span="6">
          ...
          <el-row :gutter="10" class="row">
            ...
            <el-col :span="6">
              <div class="grid-content bg-purple">买家留言：{{shippingData.memo}}</div>
            </el-col>
            <el-col :span="6">
          ...
          <el-table :data="gridData" class="table-class">
            ...
            <el-table-column property="address" label="规格">
              <template slot-scope="scope"
                v-if="!scope.row.isTotal">{{scope.row.fullName.indexOf('[')!==-1?scope.row.fullName.split("[")[1].split("]")[0]:''}}</template>
            </el-table-column>
            <el-table-column property="price" label="单价">
              <template slot-scope="scope" v-if="!scope.row.isTotal">￥{{(scope.row.price*1).toFixed(4)}}</template>
            </el-table-column>
...
export default {
  ...
  methods: {
      ...
      http
        .get(
          window.top.dType === 'purchaseOrder'
            ? '/admin/purchaseOrderManage/product/list.jhtml'
            : urls.memberCardMgmt.memberCard.listProduct,
          params
        )
        .then(data => {
      ...
      http
        .get(
          window.dType === 'purchaseOrder'
            ? logisticsMgmt.delivery.queryShippingVoucherPurchase
            : logisticsMgmt.delivery.queryShippingVoucher,
          params
        )
        .then(data => {
    ...
    handleSelect(val) {
      ...
      if (!contain) {
        this.tableData.push(Object.assign(val, { product: { minimumPackQuantity: val.minimumPackQuantity, barCode: val.barCode } }));
      }
    },
    deleteHandeler(val) {
