<template>
  <biz-grid class="order-base-info">
    ...
    <el-form ref="form" :model="form" label-width="120px" size="small" v-loading="loading" :disabled="!isEdit" :inline-message="true">
      ...
      <el-form-item label="订单原价" prop="originalPrice" v-if="showInfo">
        <el-input v-model="form.originalPrice" :disabled="true"></el-input>
      </el-form-item>
      ...
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="2">
          ...
          <el-form-item label="图片信息" v-if="this.$route.params.orderType==='rescue'" class="imgUpload">
            <el-upload action="/admin/file/uploads.jhtml" :before-upload="beforeAvatarUpload" :file-list="imageList"
              list-type="picture-card" name="file" accept="image/*" :data="{type:'0'}"
          ...
          <el-form-item label="服务日期" prop="orderItems[0].appointment.appontmentDate" v-if="this.$route.params.orderType==='ride'">
            <el-date-picker v-model="form.orderItems[0].appointment.appontmentDate" format="yyyy-MM-dd"
              value-format="yyyy-MM-dd HH:mm:ss" type="date" placeholder="请选择服务日期"></el-date-picker>
          </el-form-item>
          <el-form-item label="开始时间" prop="orderItems[0].appointment.driveStartDate" v-if="this.$route.params.orderType==='ride'">
            <el-time-picker v-model="form.orderItems[0].appointment.driveStartDate" format="HH:mm:ss"
              value-format="yyyy-MM-dd HH:mm:ss" placeholder="请选择开始时间" :disabled="!!itemId&&handler!=='service'"></el-time-picker>
          </el-form-item>
          ...
          <el-form-item label="体验车号" prop="orderItems[0].appointment.plateNumber" v-if="this.$route.params.orderType==='ride'"
            class="isRequired">
            ...
            <el-autocomplete v-model="form.orderItems[0].appointment.plateNumber" :fetch-suggestions="querySearchAsync"
              placeholder="请输入体验车号" @select="handleSelect"></el-autocomplete>
          </el-form-item>
          <el-form-item label="体验车型" prop="orderItems[0].appointment.product.fullName"
            v-if="this.$route.params.orderType==='ride'" class="isRequired">
            <el-input v-model="form.orderItems[0].appointment.product.fullName" @focus="queryProduct" placeholder="请选择体验车型">
            </el-input>
          </el-form-item>
        ...
        <el-collapse-item name="4" v-if="showPayment">
          ...
          <el-form-item label="支付方式" prop="paymentMethodId" class="isRequired" v-if="itemId">
            <el-select v-model="form.paymentMethodId" :disabled="form.type!=='1'&&form.type!=='2'" placeholder="请选择支付方式">
        ...
        <!-- <el-collapse-item name="5">
          ...
          <el-form-item label="零担物流" prop="shippings[0].isLCL" v-if="form.shippingMethodName&&showDelivery">
            <el-switch v-model="form.shippings[0].isLCL" :disabled="itemId?handler!=='shipping' || isShipped:false" active-value="1" inactive-value="0" active-color="#ff7300" inactive-color="#d9d9d9"></el-switch>
          </el-form-item>
          <el-form-item label="物流公司" prop="shippings[0].deliveryCorpId" v-if="form.shippingMethodName&&showDelivery"
            ...
            <el-select v-model="form.shippings[0].deliveryCorpId" filterable
              :disabled="itemId?handler!=='shipping' || isShipped || form.shippings[0].isLCL==='1':false" placeholder="请选择物流公司">
          ...
          <el-form-item label="物流单号" prop="shippings[0].expressSn" v-if="form.shippingMethodName&&showDelivery"
            class="isRequired">
            <el-input v-model="form.shippings[0].expressSn" :disabled="itemId?handler!=='shipping' || isShipped || form.shippings[0].isLCL==='1':false"
  ...
  methods: {
    ...
    getCurAmount() {
      if (this.form.offsetAmountMethod === '0') {
        this.form.curAmount = (Number(this.form.originalPrice) + Number(this.form.offsetAmount || '0')).toFixed(2);
        if (this.form.curAmount <= 0) {
          this.form.curAmount = 0.0;
        }
        this.form.curAmount = this.form.curAmount.toString();
      } else {
        this.form.curAmount = (Number(this.form.originalPrice) * Number(this.form.offsetAmount / 100 || '1')).toFixed(2);
        if (this.form.curAmount <= 0) {
          this.form.curAmount = 0.0;
        }
        this.form.curAmount = this.form.curAmount.toString();
      ...
      service.viewOrder(
        ...
        data => {
          ...
          if (this.form.offsetAmountMehthod === '0') {
            this.form.curAmount = Number(this.form.originalPrice) + Number(this.form.offsetAmount);
          } else {
          ...
          service.createOrder(
            {
              ...
              offsetAmountMethod: this.form.offsetAmountMethod,
              offsetAmountRate: this.form.offsetAmount,
              AdjustPriceRate: this.form.AdjustPriceRate,
