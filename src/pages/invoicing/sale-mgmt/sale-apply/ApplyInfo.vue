  ...
  <biz-grid class="apply-info">
      ...
      <div class="apply-info-content">
        <div class="apply-top-content">
          ...
          <div class="receiver-info">
            <span>{{orderData.receiver?orderData.receiver.consignee:''}}</span>
            <span>{{orderData.receiver?orderData.receiver.phone:''}}</span>
            <p>
              <i style="margin-right:5px;" class="el-icon-location-outline"></i>
              {{orderData.receiver?orderData.receiver.address:''}}
            </p>
          </div>
        </div>
        <div class="apply-info-new">
          <div class="shop-name" @click="goSupplier">
            <img :src="iconShop">
            <span style="fontSize:14px">{{orderData.shop?orderData.shop.name:''}}</span>
          </div>
          <div v-for="(item, index) of orderData.itemDatas" :key="index">
            <div class="good-info">
              <div class="goods-img">
                <img :src="item.thumbnail&&item.thumbnail.indexOf('/')>=0?item.thumbnail:defaultImg" @error="imgError(index)" class="img"
                  ref="img">
              </div>
              <div class="good-right">
                <div class="good-name">
                  <span style="flex:1">{{item.specValue}}
                  </span>
                </div>
    ...
    <div slot="bottom" v-if="isPend" style="float: right;">
      <el-button type="primary" size="mini"
        ...
        @click.stop="toPass(orderData,'1')">审核</el-button>
      <el-button type="primary" size="mini" v-if="orderData.state==='0'" @click.stop="statusChange(orderData,'0')">提交</el-button>
      <el-button type="primary" size="mini" v-if="orderData.state==='1'&&orderData.isApplier==='1'"
        @click.stop="statusChange(orderData,'3')">撤回</el-button>
      <!-- <el-button
        type="primary"
        size="mini"
        v-if="(orderData.state==='1'&& orderData.audit==='1') || (orderData.state==='2'&& orderData.audit==='1')"
