  ...
  <biz-grid class="order-product-info">
    ...
    <biz-list ref="table" :tableData="tableData" :loading="loading" :key="Math.random()">
      ...
      <el-table-column prop="subTotal" label="小计(元)">
        <div slot-scope="scope">
          <div class="biz-customcol">{{scope.row.quantity?(Number(scope.row.quantity)*Number(scope.row.price)).toFixed(2):''}}</div>
        </div>
      </el-table-column>
      ...
      <vue-easy-print tableShow ref="easyPrint">
        <div class="d-p-wrap">
          ...
          <el-row :gutter="10" class="row-top">
            ...
            <el-col :span="6">
              <div class="grid-content bg-purple">配送方式：{{formData.shippingMethodName}}</div>
            </el-col>
            <el-col :span="6">
              <div class="grid-content bg-purple">收货人：{{formData.consignee}}</div>
            </el-col>
            <el-col :span="6">
              <div class="grid-content bg-purple">电话：{{formData.phone}}</div>
            </el-col>
          </el-row>
          <el-row :gutter="10" class="row">
            <el-col :span="12">
              <div class="grid-content bg-purple">地址：{{formData.areaName? formData.areaName + formData.address : formData.address}}</div>
            </el-col>
            <el-col :span="6">
...
export default {
  ...
  methods: {
    ...
    certificate() {
      ...
      http
        .get(logisticsMgmt.delivery.queryShippingVoucherPurchase, params)
        .then(data => {
    ...
    getList() {
      ...
      http
        .get('admin/purchaseOrderManage/products.jhtml', params)
        .then(data => {
          this.loading = false;
