  ...
  <div class="cart-product-list">
    ...
    <div class="cart-product-item" v-for="(shop, shopIndex) in list" :key="shopIndex">
      ...
      <div class="product-list">
        <div class="shop">
          <div class="check" slot="left" @click.stop="handleShopCheckClick(shop, shopIndex)">
            <span
              class="iconfont biz-icon-i-sh-man-xz"
              v-show="selectionKeys[shopIndex] && shop.cartItems && selectionKeys[shopIndex].length === shop.cartItems.length"
            ></span>
            <span
              class="iconfont biz-icon-i-sh-man-wxz"
              v-show="!selectionKeys[shopIndex] || !shop.cartItems || selectionKeys[shopIndex].length !== shop.cartItems.length"
            ></span>
          </div>
          <div class="title">{{shop.shop ? shop.shop.name : ''}}</div>
        ...
        <hy-image-panel
          v-for="(item, itemIndex) in shop.cartItems"
          :key="itemIndex"
        ...
        >
          ...
          <div class="product-item">
            <div class="title">
              <span>{{item.product.fullName.length>35?item.product.fullName.substr(0, 35) + '...':item.product.fullName}}</span>
              <!-- <span class="preorder" v-show="item.product.categoryType === '2'">可预约</span> -->
            </div>
            <!-- <div class="info-bottom">
              <div style="opacity:0.65">{{item.product.fullName&&item.product.fullName.indexOf('[')!==-1?item.product.fullName.split("[")[1].split("]")[0]:''}}</div>
            </div> -->
            <div class="info-bottom" style="display:flex">
              <div>{{item.product&&item.product.barCode?item.product.barCode:''}}</div>
              <div style="position: absolute;left: auto;right: 0;">最小包装数：{{item.product&&item.product.minimumPackQuantity?item.product.minimumPackQuantity:''}}</div>
            </div>
            <div class="price">￥{{item.effectivePrice}}</div>
            <div class="quantity">x {{item.quantity}}</div>
            <hy-stepper class="quantity" v-model="item.quantity" :min="1" :cartStyle="true" @click.native.stop @input="handleCount(item)">></hy-stepper>
          </div>
        </hy-image-panel>
      </div>
    </div>
