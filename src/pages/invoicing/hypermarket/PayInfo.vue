<template>
  <biz-grid class="payInfo" v-loading="loading">
      ...
      <div class="item">
        ...
        <div v-if="paymentType.type==='2'&&radio==='2'">
          <el-select v-model="memberCardId" placeholder="请选择会员卡" size="mini">
            <el-option v-for="memberCard in paymentType.memberCards" :key="memberCard.id"
              :label="memberCard.template.name+'  余额'+'￥'+memberCard.balance" :value="memberCard.id"></el-option>
          </el-select>
        </div>
      </div>
    </div>
    <el-form label-width="140px" size="small" ref="form" :model="form" :rules="rules" :inline-message="true">
      <el-form-item label="会员卡支付密码" prop="password" v-if="radio==='2'">
        <div style="display:flex;">
          <el-input type="password" v-model="form.password" maxlength="6" minlength="6" size="mini"></el-input>
    ...
    <el-dialog :visible.sync="dialogVisible" title="设置支付密码" width="40%" append-to-body>
      <div class="valid-div">
        <el-form label-width="140px" size="small" ref="formPass" :model="formPass" :rules="rulesPass" :inline-message="true">
