<template>
  <biz-grid class="add-step-3">
    <div slot="top">
      <div class="header-step">
        <div class="step-2" @click="stepToTwo">
          <span>①</span>
          <span>选择商品</span>
          <span>></span>
        </div>
    ...
    <div class="order-confirm">
      <div class="order-confirm-content">
        ...
        <div class="order-goods">
          <div class="shop-name">
            <img :src="iconShop" />
            <span style="fontSize:14px">{{orderConfim.products?orderConfim.products[0].shopName:''}}</span>
          </div>
          <div v-for="(item,index) of orderConfim.products" :key="index">
            <div class="order-goods-div">
              <div class="img-div">
                <img :src="item.product&&item.product.image?item.product.image:vfacLogo" @error="imgError" class="img" />
              </div>
      ...
      <el-dialog :visible.sync="dialogVisiableInvoice" append-to-body custom-class="bill-dialog">
        <invoice-info @selectedItem="selectedItem"></invoice-info>
      </el-dialog>
    </div>
  </biz-grid>
</template>
...
export default {
  ...
  methods: {
    ...
    calculateOrder(bean = '0') {
      ...
      http
        .post(urls.hypermarketMgmt.calculate, param)
        .then(data => {
    ...
    createOrder() {
      ...
      let param = {
        ...
        receiverId: this.showAddress && !this.priceCoupon ? this.receiverId : '',
      ...
      let obj;
      let product;
      let amountCreate = this.getAmount;
      for (let i = 0; i < len; i++) {
        obj = {};
        obj.id = ary[i].id;
        obj.quantity = ary[i].quantity;
      ...
      // for (const item of param.cartItems) {
      //   amountCreate += item.amount;
      // }
      http
        .post('/admin/purchaseOrder/agent/order.jhtml', param)
        .then(data => {
          ...
          if (this.payment.name === '在线支付') {
            if (parseFloat(this.getRealAmount) !== 0) {
              ...
              this.$emit('step3Confirm', data.id, amountCreate);
            } else {
              // this.payNow();
              this.$emit('complate', data.id);
            }
          } else {
            this.$emit('complate', data.id);
          }
