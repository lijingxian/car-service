<template>
  <biz-grid class="order-info">
    <div class="order-info-div">
      <div class="order-info-content">
        <div class="order-top-content">
          <div class="order-follow-up" @click="onOrderTrack">
            <span>订单跟踪</span>
            ...
            <div class="good-info" @click="onClickProduct(item)">
              ...
              <div class="good-right">
                <div class="good-name">
                  ...
                  <div class="btn-group-div" v-if="showRefund[index]">
                    <div class="btn-group">
                      <span class="btn-good-pay" @click.stop="onRefund(item)" v-if="showRefund[index]">{{refundsLabel[index]}}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="order-bottom">
        ...
        <div class="btn-group">
          <!-- <span class="btn-serv" @click="onConsultation">客服咨询</span> -->
          <!-- <span class="btn-orange-pay" @click="onCreateInvoice" v-if="(this.orderInfo.paymentStatus === '2' || this.orderInfo.paymentStatus === '3') && parseFloat(this.orderInfo.amount) > 0 &&!this.orderInfo.invoiceTitle">补开发票</span>
          <span class="btn-orange-pay" @click="onCancelPay" v-if="showCancelOrder">取消订单</span>
          <span class="btn-pay" @click="onPay" v-if="showPayment">立即支付</span>
          <span class="btn-pay" @click="onPay" v-if="showPayDeposit">支付定金</span>
          <span class="btn-pay" @click="onPay" v-if="showFinalPayment">支付尾款</span>
          <span class="btn-orange-pay" @click="onLookLogistics" v-if="showLogistics">物流跟踪</span>
          <span class="btn-pay" @click="onConfirmReceive" v-if="showReceive">确认收货</span>
          <span class="btn-pay" @click="onEvaluate" v-if="showEvaluate">立即评价</span>
          <span class="btn-pay" @click="onLookEvaluate" v-if="showLookEvaluate">查看评价</span>-->
        </div>
      </div>
    ...
    <div slot="bottom" style="float: right;">
      ...
      <el-button
        ...
        v-if="(orderInfo.state==='4'&&orderInfo.shippingWay==='0')||(orderInfo.state==='3'&&orderInfo.shippingWay==='1')"
      >确认收货</el-button>
      <el-button type="primary" size="mini" @click.stop="onReview(orderInfo.id,orderInfo.state)" v-if="orderInfo.state==='5'">立即评价</el-button>
    ...
    <el-dialog :visible.sync="dialogVisiableLog" append-to-body custom-class="bill-dialog">
      <order-track ref="log"></order-track>
    </el-dialog>
    <el-dialog :visible.sync="dialogApplaySale" title="售后" center append-to-body>
      <apply-sale @operationSuccess="operationSuccess" :isEditable="isEditable" :itemId="orderItemId" :dialogApplaySale="dialogApplaySale" ref="applysale"></apply-sale>
    </el-dialog>
    <el-dialog :visible.sync="dialogApplayRefundInfo" append-to-body>
      <refund-info @operationSuccess="operationSuccess" :itemId="itemId" :dialogApplayRefundInfo="dialogApplayRefundInfo" @applySaleHanlder="applySaleHanlder"></refund-info>
    </el-dialog>
  </biz-grid>
</template>
