      ...
      <div class="apply-info-content">
        ...
        <div class="apply-info-new">
          <div class="shop-name" @click="goSupplier">
            <img :src="iconShop" />
            <span style="fontSize:14px">{{orderData.shop.name}}</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    getOrderInfo() {
      ...
      http
        .get('/admin/invoicing/purchase/agentPurchaseApply.jhtml', param)
        .then(data => {
    ...
    statusChange(model, state) {
      this.$confirm('确定执行此操作?', '提示', {
        ...
        http
          .post(urls.invoicing.purchaseApplyMgmt.state, params)
          .then(data => {
