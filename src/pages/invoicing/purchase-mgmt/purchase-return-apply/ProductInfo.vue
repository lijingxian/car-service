<template>
  <biz-grid class="product-info">
    ...
    <biz-list ref="table" :tableData="tableData" :height="'100%'" :loading="loading" :key="Math.random()">
      ...
      <el-table-column prop="price" label="采购单价(元)" min-width="150">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      http
        .get(urls.invoicing.purchaseReturnApplyMgmt.shopPurchaseReturnApplyItems, params)
        .then(data => {
    ...
    deleteHandeler(val) {
      if (val.row.id === '') {
        ...
        this.$confirm('是否确认删除?', '提示', {
          ...
          this.loading = true;
          http
            .delete(urls.invoicing.purchaseReturnApplyMgmt.shopPurchaseReturnApplyItem, { ids: [val.row.id] })
            .then(data => {
    ...
    save() {
      ...
      this.tableData.forEach(item => {
        ...
        if (item.price === '' || item.depotId === '' || item.depotId === undefined) {
      ...
      if (flag) {
        ...
        http
          .post(urls.invoicing.purchaseReturnApplyMgmt.shopPurchaseReturnApplyItem, params)
          .then(data => {
            this.$message.success('保存成功');
            this.$emit('setOrderItems', this.itemDatas);
