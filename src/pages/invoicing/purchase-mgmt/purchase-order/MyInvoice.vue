<template>
  <biz-grid class="my-invoice-my-purchase">
    <div class="invoice-info-my-purchase">
      ...
      <el-form label-width="100" :inline-message="true">
        ...
        <el-form-item label="所在地区：">{{curInvoice.areaName}}</el-form-item>
        <el-form-item label="详细地址：">{{curInvoice.address}}</el-form-item>
        <el-form-item label="快递公司：" v-if="expressName">{{expressName}}</el-form-item>
        <el-form-item label="快递单号：" v-if="curInvoice.expressCode">{{curInvoice.expressCode}}</el-form-item>
      </el-form>
    ...
    <el-button type="primary" size="mini" @click="submit" v-if="invoiceInfoId&&hasAuthEdit">提交</el-button>
    <el-button type="primary" size="mini" @click="cancle" v-if="invoiceInfoId&&hasAuthEdit">取消</el-button>
  </biz-grid>
</template>
