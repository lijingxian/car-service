          ...
          <el-row :gutter="10" class="row">
            <el-col :span="14">
              <div class="grid-content bg-purple">
                发货时间：{{shippingsPrint.shippingDate?shippingsPrint.shippingDate.split(' ')[0]:''}}</div>
            </el-col>
            <el-col :span="10">
...
export default {
  ...
  watch: {},
  methods: {
    ...mapMutations('Order', ['updateLucidLogs']),
    // subRightBtnTxt(item) {
    //   if (item.content === '交车') {
    //     return '查看交车报告';
    //   }
        ...
        this.$emit(
          ...
          this.orderItems.length > 0
            ? this.orderItems[0].product
              ? this.orderItems[0].product.vins.length > 0
                ? this.orderItems[0].product.vins[0].number
                : ''
              : ''
            : ''
        );
    ...
    getLogs(orderId) {
      ...
      http
        .get('/admin/purchaseOrder/orderLogs.jhtml', param)
        .then(data => {
          // this.logs = data.logs;
          // let ary = this.logs;
          // let len = ary.length;
          ...
          // for (let i = 0; i < len; i++) {
          //   // 执行步骤
          //   if (ary[i].createTime || (ary[i].content === '执行' && data.serviceLogs.length)) {
          //     ary[i].status = 'cur';
          //   } else {
          //     ary[i].status = 'no-started';
          //   }
          // }
          ...
          this.orderItems = data.shippings && data.shippings.length > 0 && data.shippings[0].shippingItems;
          ...
          this.devProcessShippings = data.shippings && data.shippings.length > 0 && data.shippings[0];
          if (data.payments) {
            this.paymentType = data.payments.length > 0 ? data.payments[0].paymentType : '';
            this.paymentMethod = data.payments.length > 0 ? data.payments[0].paymentMethod : '';
          ...
          this.updateLucidLogs(this.serviceLogs);
        })
        .catch(err => {
          console.log('ERR: ', err);
        })
