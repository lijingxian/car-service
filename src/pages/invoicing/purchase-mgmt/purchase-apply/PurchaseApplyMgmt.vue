  ...
  <div class="purchase-apply-mgmt">
        ...
        <template #default="scope">
          <biz-column-operation :row="scope.row">
            <el-button
              ...
              v-if="hasAuthExamine&&((scope.row.state==='1'&& scope.row.audit==='1') || (scope.row.state==='2'&& scope.row.audit==='1'))"
            >审核</el-button>
            ...
            @click.stop="reject(scope.row,'2')">驳回</el-button>-->
    ...
    <biz-popup-tabs :open="openAdd" @close="openAdd=false" v-model="tabName">
      <el-tab-pane label="添加申请单" name="addinfo">
        <base-info v-if="step1" ref="step1Info" @step1Confirm="step1Confirm" :backForm="backForm"></base-info>
...
export default {
      ...
      operations: [
        {
          ...
          type: 'primary',
          class: 'show',
          auth: ['admin:purchaseApplication.add']
        },
        {
          label: '删除',
          name: 'delete',
          type: '',
          disabled: true,
          auth: ['admin:purchaseApplication.delete']
        },
        {
          label: '刷新',
          name: 'refresh',
        ...
        },
        {
          ...
          name: 'exportStart',
          type: '',
          auth: ['admin:purchaseApplication.export']
        }
      ],
      filters: [
        {
  ...
  methods: {
    ...
    supplierClick(row) {
      this.supplierId = { id: row.relatePartnerId };
      this.isEdit = false;
    ...
    exportStart() {
      ...
      http
        .post('/admin/invoicing/purchase/shopPurchaseApplies/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/invoicing/purchase/shopPurchaseApplies/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
        console.log('response => ', response);
        const blob = new Blob([response.data], { type: 'application/vnd.ms-excel' });
        console.log(decodeURI(response.headers['content-disposition']));
        const fileName = decodeURI(response.headers['content-disposition'].split('filename=')[1]);
    ...
    getList() {
      ...
      this.loading = true;
      http
        .post(urls.invoicing.purchaseApplyMgmt.shopPurchaseApplyList, params)
        .then(data => {
          console.log(data);
    ...
    queryKpi() {
      ...
      http
        .get('/admin/invoicing/purchase/queryShopPurchaseKpi.jhtml', params)
        .then(data => {
          this.$message({
