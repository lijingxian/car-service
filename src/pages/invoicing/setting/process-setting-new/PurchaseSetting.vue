<template>
  <biz-grid class="purchase-setting">
    <div slot="top">
      <div class="slot-top">
        ...
        <div class="model">
          <div class="font">采购流程模版</div>
          <img :src="cglc" height="150">
        </div>
      </div>
    </div>
    <biz-flex-table ref="table" :tableData="tableData" :loading="loading" :pagination="pagination" v-show="!showList"
      ...
      <el-table-column label="流程" min-width="700">
        <template slot-scope="scope">
          <div class="process">
            <img :src="cglc" height="150" align="center" v-if="scope.row.configs&&scope.row.configs[0].state==='1'">
            <img :src="cglc1" height="150" align="center" v-if="scope.row.configs&&scope.row.configs[0].state==='0'">
    ...
    <biz-flex-table ref="table" :tableData="tableData" :loading="loading" :pagination="pagination" v-show="showList"
      ...
      <el-table-column label="提交采购申请" min-width="100">
        <template slot-scope="scope">
          <div>
            ...
            <template v-if="scope.row.configs[0].state==='1'">
              <div v-for="item in scope.row.configs[0].items" :key="item.id">
                {{item.role.name?item.role.name+':':'全部角色'}}
                {{item.role.name?(item.approver.name?item.approver.name:'全部'):''}}
              </div>
            </template>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="采购申请审批" min-width="100">
        <template slot-scope="scope">
          <div v-if="scope.row.configs[0].state==='0'">✖</div>
          <template v-if="scope.row.configs[0].state==='1'">
      ...
      <el-table-column label="提交采购订单" min-width="100">
        <template slot-scope="scope">
          <div>
      ...
      <el-table-column label="采购订单审批" min-width="100">
        <template slot-scope="scope">
    ...
    <div slot="float">
      <biz-popup-tabs :open="open" @close="open = false;" v-model="tabName">
        <el-tab-pane label="设置" name="first">
          <purchase-setting-info :idObject="infoId" @item-updated="refresh"></purchase-setting-info>
        </el-tab-pane>
      </biz-popup-tabs>
    </div>
  </biz-grid>
...
import cglc from '@/assets/images/process/03.png';
import cglc1 from '@/assets/images/process/04.png';
import http from '@/common/http';
import urls from '@/common/urls';
import { BizPopupTabs } from '@/components/BizPopup';
import { BizHeader, BizFlexTable } from '@/components/BizTable';
import PurchaseSettingInfo from './PurchaseSettingInfo';
export default {
  ...
  methods: {
    formatRow(row) {
      if (row.configs[0].items.length === 1) {
        if (row.configs[0].items[0].role.name) {
          return (row.configs[0].items[0].role.name + (row.configs[0].items[0].approver.name ? row.configs[0].items[0].approver.name : '全部')
          );
        } else {
          return '全部角色';
        }
        ...
        if (row.configs[0].items[0].role.name) {
          return (row.configs[0].items[0].role.name + (row.configs[0].items[0].approver.name ? row.configs[0].items[0].approver.name : '全部') + '等');
        } else {
          return '全部角色等';
        }
      }
