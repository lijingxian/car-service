<template>
  <biz-grid class="purchase-setting-info">
    <div slot="top" class="popup">
      <biz-save-button slot="top" :operations="operations" @operate="operate"></biz-save-button>
    </div>
    <el-form ref="form" label-width="80px" size="small" v-loading="loading" :model="formData" :rules="rules">
      <el-form-item label="流程名称" prop="name">
        <el-input v-model="formData.name" placeholder="代理商采购流程"></el-input>
      </el-form-item>
      ...
      <div class="image">
        <img :src="xslc">
        <div class="step1" @click.stop="step1Click">提交采购申请</div>
        <div :class="formData.configs[0].state==='1'?'step2':'step2-disabled'" @click.stop="step2Click">采购申请审批</div>
        <div class="step3" @click.stop="step3Click">提交采购订单</div>
        <div class="step4" @click.stop="step4Click">采购订单审批</div>
      </div>
      <div class="content">
        ...
        <!-- <el-form-item v-if="step4" label-width="130" prop="autoNext" class="isRequired">
        ...
        <el-form-item label="审批流程" v-if="(step2&&formData.configs[1].way==='1')||(step4&&formData.configs[3].way==='1')"
          class="isRequired">
              ...
              <div slot="description" class="stepItem">
                <el-form ref="processForm">
                  <el-form-item prop="process">
                    <el-cascader :options="serviceDatas" :props="cascaderProps" v-model="list.ids"
        ...
        .then(data => {
          ...
          this.formData.configs[0].items.map(item => {
            ...
            if (item.role.id) {
              ...
              this.getChildList([item.role.id]);
              if (item.approver.id) {
