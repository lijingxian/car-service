  ...
  <div class="product-analysis">
    <div slot="top">
      <show-search-grid @refreshHandle="refreshHandle" ref="search">
        <div slot="description" class="description-pover">
          <p>售后类型：统计采购订单商品售后类型</p>
          <p>售后频次：统计采购订单商品售后次数</p>
          <p>售后率：统计采购订单商品售后率</p>
        </div>
      </show-search-grid>
    </div>
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="purchaseRefundsType">售后类型</el-radio-button>
              <el-radio-button label="purchaseRefundsNum">售后频次</el-radio-button>
              <el-radio-button label="purchaseRefundsRate">售后率</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;height:600px;overflow:auto" v-show="radioValue==='0'&&indicatorValue==='purchaseRefundsType'">
          <div v-for="(list,index) in channelLists" :key="index">
            <span :class="'list'+index">
              {{list.xValue}}:{{list.y[0].yValue}}单
              占比{{countTotal!==0?((list.y[0].yValue/countTotal)*100).toFixed(2):list.y[0].yValue}}%
            </span>
          </div>
        </div>
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue!=='purchaseRefundsType'"></biz-chart>
        <biz-chart :chartData="chartDataOfWork" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='purchaseRefundsType'"></biz-chart>
      <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%;height:100%" v-show="radioValue==='1'">
          ...
          <el-table-column prop="productQuantity" label="采购数量"></el-table-column>
          <el-table-column prop="afterSalesNum" label="售后次数"></el-table-column>
          <el-table-column prop="refundsNum" label="退款"></el-table-column>
          <el-table-column prop="returnsNum" label="退款退货"></el-table-column>
          <el-table-column prop="returnGoodsNum" label="退货"></el-table-column>
          <el-table-column prop="exchangeNum" label="换货"></el-table-column>
          <el-table-column prop="refundsRate" label="售后率"></el-table-column>
        </biz-list>
...
export default {
  ...
  data() {
    return {
      ...
      countTotal: 0,
      radioValue: '0',
      indicatorValue: 'purchaseRefundsType'
    };
  },
  props: {
    params: {
  ...
  methods: {
    ...
    getGraphData() {
      ...
      http
        ...
        .then(data => {
          if (this.indicatorValue === 'purchaseRefundsType') {
            this.countTotal = 0;
          ...
          } else if (this.indicatorValue === 'purchaseRefundsNum') {
            this.chartData = this.dataFormatZf(data);
          } else {
            this.chartData = this.dataFormatZfA(data);
          }
        })
        .catch(errData => {
          this.loading = false;
    ...
    dataFormatZf(data) {
      ...
      for (let i = 0; i < (data.indicators.length > 10 ? 10 : data.indicators.length); i++) {
        tmpData.push({
          ...
          次: this.indicatorValue === 'purchaseRefundsNum' ? data.indicators[i].y[0].yValue : data.indicators[i].y[2].yValue
        });
      }
      let chartData = {
        legendVisible: false,
    ...
    dataFormatZfA(data) {
      ...
      for (let i = 0; i < (data.indicators.length > 10 ? 10 : data.indicators.length); i++) {
        tmpData.push({
          商品: data.indicators[i].xValue,
          占比: this.indicatorValue === 'purchaseRefundsNum' ? data.indicators[i].y[1].yValue : data.indicators[i].y[2].yValue
        });
      }
      let chartData = {
        legendVisible: false,
