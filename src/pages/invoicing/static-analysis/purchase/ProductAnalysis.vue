  ...
  <div class="product-analysis">
    <div slot="top">
      <show-search-grid @refreshHandle="refreshHandle" ref="search">
        <div slot="description" class="description-pover">
          <p>分类：统计采购商品分类的分布情况</p>
          <p>数量：统计采购商品的数量变化趋势</p>
          <p>排行：统计商品的采购排行情况</p>
        </div>
      </show-search-grid>
    </div>
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              ...
              <el-radio-button label="purchaseProductCategory">分类</el-radio-button>
              <el-radio-button label="purchaseProductTop">排行</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue!=='purchaseProductCategory'"></biz-chart>
        <biz-chart :chartData="chartDataOfWork" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='purchaseProductCategory'"></biz-chart>
  ...
  methods: {
    ...
    getGraphData() {
      ...
      http
        .post(url, params)
        .then(data => {
          if (this.indicatorValue === 'purchaseProductCategory') {
            this.countTotal = 0;
          ...
          } else if (this.indicatorValue === 'purchaseProductNum') {
    ...
    dataFormatZf(data) {
      ...
      for (let i = 0; i < (data.indicators.length > 10 ? 10 : data.indicators.length); i++) {
        tmpData.push({
          ...
          个: data.indicators[i].y[0].yValue
        });
      }
      let chartData = {
        legendVisible: false,
