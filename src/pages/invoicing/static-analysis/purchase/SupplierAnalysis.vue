  ...
  <div class="supplier-analysis">
    <div slot="top">
      <show-search-grid @refreshHandle="refreshHandle" ref="search">
        <div slot="description" class="description-pover">
          <p>订单数量：统计向供应商采购订单数量情况</p>
          <p>订单金额：统计向供应商采购订单金额情况</p>
          <p>生产周期：统计供应商供货周期情况</p>
        </div>
      </show-search-grid>
    </div>
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="supplierOrderNum">订单数量</el-radio-button>
              <el-radio-button label="supplierOrderAmount">订单金额</el-radio-button>
              <el-radio-button label="supplierProductionCycle">生产周期</el-radio-button>
              <el-radio-button label="supplierProductNum">商品情况</el-radio-button>
              <el-radio-button label="supplierShippingCycle">发货周期</el-radio-button>
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;height:600px;overflow:auto" v-show="radioValue==='0'&&indicatorValue==='supplierProductNum'">
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-show="radioValue==='1'&&(indicatorValue==='supplierOrderNum'||indicatorValue==='supplierOrderAmount')">
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-show="radioValue==='1'&&(indicatorValue==='supplierProductionCycle'||indicatorValue==='supplierShippingCycle')">
          ...
          <el-table-column prop="productionCycle" label="生产周期"></el-table-column>
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-show="radioValue==='1'&&indicatorValue==='supplierProductNum'">
          <el-table-column prop="supplierName" label="供应商"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      countTotal: 0,
      radioValue: '0',
      indicatorValue: 'supplierOrderNum'
    };
  },
  props: {
    params: {
  ...
  methods: {
    ...
    getSupplierList() {
      ...
      http
        ...
        .then(data => {
          ...
          this.checkedSupplier = this.params.checkedSupplier;
          let checkedCount = this.checkedSupplier.length;
          this.checkAllSupplier = checkedCount === this.conditionData.supplierList.length;
          this.isIndeterminateSupplier = checkedCount > 0 && checkedCount < this.conditionData.supplierList.length;
          this.query();
        })
    ...
    getStatesList() {
      ...
      http
        ...
        .then(data => {
          ...
          this.checkedState = this.params.checkedState;
    ...
    getGraphData() {
      ...
      if (this.indicatorValue === 'supplierProductNum' || this.indicatorValue === 'supplierProductionCycle' || this.indicatorValue === 'supplierShippingCycle') {
      ...
      http
        .post(url, params)
        .then(data => {
          if (this.indicatorValue === 'supplierOrderNum' || this.indicatorValue === 'supplierOrderAmount') {
            this.chartData = this.dataFormat(data);
          } else if (this.indicatorValue === 'supplierProductNum') {
            this.countTotal = 0;
    ...
    getTable() {
      let params = {
        ...
        viewType: this.viewType,
        beginDate: this.beginDate,
        dimensions: this.indicatorValue === 'supplierProductNum' ? ['productId', 'relatePartnerId'] : [],
        endDate: this.endDate,
