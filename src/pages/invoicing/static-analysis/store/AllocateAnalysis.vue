<template>
  <div class="allcate-analysis">
    ...
    <div class="indicators-search">
      <div class="title">调出仓库</div>
        <el-checkbox-button v-model="checkAllSupplier" @change="handleCheckAllSupplier" :indeterminate="isIndeterminateSupplier">全部
        </el-checkbox-button>
        <el-tooltip placement="bottom" v-for="(item, index) in storeList" :key="index" :disabled="item.name.length<=10">
          <el-checkbox-group v-model="checkedSupplier" @change="supplierChange">
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="depotTransferNum">调拨次数</el-radio-button>
              <!-- <el-radio-button label="depotTransferProductNum">数量</el-radio-button> -->
              <el-radio-button label="depotTransferDist">状态</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;height:600px;overflow:auto" v-show="radioValue==='0'&&indicatorValue==='depotTransferDist'">
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue!=='depotTransferDist'"></biz-chart>
        <biz-chart :chartData="chartDataOfWork" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='depotTransferDist'"></biz-chart>
      <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%;height:100%" v-show="radioValue==='1'">
          ...
          <el-table-column prop="items[0].name" label="商品"></el-table-column>
          <el-table-column prop="depotOut.name" label="调出仓库"></el-table-column>
          <el-table-column prop="depotIn.name" label="调入仓库"></el-table-column>
          <el-table-column prop="items[0].quantity" label="调拨数量" v-if="indicatorValue==='depotTransferProductNum'"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      // indicator
      conditionData: {
        stateList: [{ id: '0', name: '未确认' }, { id: '1', name: '已确认' }],
        supplierList: []
      },
      ...
      countTotal: 0,
      radioValue: '0',
      indicatorValue: 'depotTransferNum'
    };
  },
  props: {
    params: {
  ...
  methods: {
    ...
    getStoreList() {
      http
        ...
        .then(data => {
          ...
          this.checkedStore = this.storeList.map(item => { return item.id; });
          let checkedCount = this.storeList.length;
          this.checkAllStore = checkedCount === this.storeList.length;
          this.isIndeterminateStore = checkedCount > 0 && checkedCount < this.storeList.length;
          this.checkedSupplier = this.storeList.map(item => { return item.id; });
          let checkedCount1 = this.storeList.length;
          this.checkAllSupplier = checkedCount1 === this.storeList.length;
          this.isIndeterminateSupplier = checkedCount1 > 0 && checkedCount1 < this.storeList.length;
          this.$nextTick(() => {
    ...
    getGraphData() {
      ...
      let params = {
        ...
        depotInIds: this.checkedStore,
        depotOutIds: this.checkedSupplier,
        viewType: this.viewType,
        beginDate: this.beginDate,
        endDate: this.endDate
      };
      if (this.indicatorValue !== 'depotTransferDist') {
        http
