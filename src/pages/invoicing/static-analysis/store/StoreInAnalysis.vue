<template>
  <div class="store-in-analysis">
    <div slot="top">
      <show-search-grid @refreshHandle="refreshHandle" ref="search">
        <div slot="description" class="description-pover">
          <p>状态：统计入库单状态分布情况</p>
          <p>业务类型：统计入库单业务类型分布情况</p>
          <p>订单数量：统计入库单数量变化趋势</p>
          <p>往来方：统计入库单往来方分布情况</p>
          <p>执行情况：统计入库单的入库情况</p>
        </div>
      </show-search-grid>
    </div>
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="depotInNum">订单数量</el-radio-button>
              <el-radio-button label="depotInStateDist">状态</el-radio-button>
              <el-radio-button label="depotInTypeDist">业务类型</el-radio-button>
              <el-radio-button label="depotInRelatePartnerDist">往来方</el-radio-button>
        ...
        <div class="content"
          ...
          v-show="radioValue==='0'&&(indicatorValue!=='depotInNum'&&indicatorValue!=='zyqk')">
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false"
          v-if="radioValue==='0'&&(indicatorValue==='depotInNum'||indicatorValue==='zyqk')"></biz-chart>
...
export default {
  ...
  methods: {
    ...
    getGraphData() {
      ...
      if (this.indicatorValue === 'depotInNum' || this.indicatorValue === 'zcqk') {
        http
          .post('/admin/ecommerce/kpi/stock/graph/depotColumn.jhtml', params)
          .then(data => {
            if (this.indicatorValue === 'depotInNum') {
