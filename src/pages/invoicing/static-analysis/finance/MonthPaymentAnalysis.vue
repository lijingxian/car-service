<template>
  <div class="monthPay-analysis-order">
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="stockRank">库存排行</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <biz-chart :chartData="chartDataOfZf" :loading="loading" :showSearch="false" v-if="radioValue==='0'"></biz-chart>
      <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%;height:100%" v-show="radioValue==='1'">
          ...
          <el-table-column prop="beginningCount" label="期初数量"></el-table-column>
          <el-table-column prop="purchaseCount" label="本期采购"></el-table-column>
          <el-table-column prop="salesCount" label="本期销售">
          </el-table-column>
          <el-table-column prop="endingCount" label="期末数量">
          </el-table-column>
        </biz-list>
...
export default {
  ...
  methods: {
    ...
    exportStart() {
      ...
      http
        .post('/admin/ecommerce/kpi/finance/export/finance/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/ecommerce/kpi/finance/export/finance/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
