  ...
  <div class="product-analysis">
    <div slot="top">
      <show-search-grid @refreshHandle="refreshHandle" ref="search">
        <div slot="description" class="description-pover">
          <p>售后类型：统计销售订单商品售后类型</p>
          <p>售后频次：统计销售订单商品售后次数</p>
          <p>售后率：统计销售订单商品售后率</p>
        </div>
      </show-search-grid>
    </div>
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="salesRefundsType">售后类型</el-radio-button>
              <el-radio-button label="salesRefundsNum">售后频次</el-radio-button>
              <el-radio-button label="salesRefundsRate">售后率</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;height:600px;overflow:auto" v-show="radioValue==='0'&&indicatorValue==='salesRefundsType'">
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue!=='salesRefundsType'"></biz-chart>
        <biz-chart :chartData="chartDataOfWork" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='salesRefundsType'"></biz-chart>
      <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%;height:100%" v-show="radioValue==='1'">
          ...
          <el-table-column prop="productQuantity" label="销售数量"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      countTotal: 0,
      radioValue: '0',
      indicatorValue: 'salesRefundsType'
    };
  },
  props: {
    params: {
  ...
  methods: {
    ...
    getGraphData() {
      ...
      http
        ...
        .then(data => {
          if (this.indicatorValue === 'salesRefundsType') {
            this.countTotal = 0;
          ...
          } else if (this.indicatorValue === 'salesRefundsNum') {
    ...
    dataFormatZf(data) {
      ...
      for (let i = 0; i < (data.indicators.length > 10 ? 10 : data.indicators.length); i++) {
        tmpData.push({
          ...
          次: this.indicatorValue === 'salesRefundsNum' ? data.indicators[i].y[0].yValue : data.indicators[i].y[2].yValue
        });
      }
      let chartData = {
        legendVisible: false,
    ...
    dataFormatZfA(data) {
      ...
      for (let i = 0; i < (data.indicators.length > 10 ? 10 : data.indicators.length); i++) {
        tmpData.push({
          商品: data.indicators[i].xValue,
          占比: this.indicatorValue === 'salesRefundsNum' ? data.indicators[i].y[1].yValue : data.indicators[i].y[2].yValue
        });
      }
      let chartData = {
        legendVisible: false,
