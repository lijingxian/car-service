<template>
  <div class="channel-analysis-order">
    <div slot="top">
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              <el-radio-button label="salesOrderNum">数量</el-radio-button>
              <el-radio-button label="salesOrderState">状态</el-radio-button>
              <el-radio-button label="salesOrderAmount">金额</el-radio-button>
              <el-radio-button label="salesOrderAgent">代理商</el-radio-button>
              <!-- <el-radio-button label="spqk">审批情况</el-radio-button> -->
            </el-radio-group>
          </div>
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;height:600px;overflow:auto" v-show="radioValue==='0'&&(indicatorValue==='salesOrderState'||indicatorValue==='salesOrderAgent')">
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&(indicatorValue==='salesOrderNum'||indicatorValue==='salesOrderAmount')"></biz-chart>
        <biz-chart :chartData="chartDataOfWork" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&(indicatorValue==='salesOrderState'||indicatorValue==='salesOrderAgent')"></biz-chart>
        <biz-chart :chartData="chartDataOfZf" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='spqk'"></biz-chart>
      <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%;height:100%" v-show="radioValue==='1'">
          ...
          <el-table-column prop="clientName" label="客户信息"></el-table-column>
          <el-table-column prop="agentName" label="代理商"></el-table-column>
          <el-table-column prop="length" label="商品信息">
          ...
          <el-table-column prop="length" label="商品价格*数量">
            <template slot-scope="scope">
              <div>
                <div v-for="orderItem in scope.row.orderItems" :key="orderItem.id">
          ...
          <el-table-column prop="amount" label="审批总时长" v-if="indicatorValue==='spqk'"></el-table-column>
          <el-table-column prop="amount" label="审批人" v-if="indicatorValue==='spqk'"></el-table-column>
          <el-table-column prop="amount" label="审批时长" v-if="indicatorValue==='spqk'"></el-table-column>
        </biz-list>
...
export default {
  ...
  methods: {
      ...
      http
        ...
        .then(data => {
          ...
          this.checkedOperator = this.operatorList
            ...
            : [];
          this.$nextTick(() => {
            this.hardGroupOperatorHeight1 = document.getElementsByClassName('hardOperatorGroup1')[3].offsetHeight;
    ...
    getGraphData() {
      this.loading = true;
      let url = '';
      if (this.indicatorValue === 'salesOrderNum' || this.indicatorValue === 'salesOrderAmount') {
      ...
      http
        ...
        .then(data => {
          ...
          } else if (this.indicatorValue === 'spqk') {
            this.chartDataOfZf = this.dataFormatZf(data);
          } else {
