<template>
  <div class="product-analysis-invoice">
    <div slot="top">
      <show-search-grid @refreshHandle="refreshHandle" ref="search">
        <div slot="description" class="description-pover">
          <p>分类：统计销售商品分类的分布情况</p>
          <p>数量：统计销售商品的数量变化趋势</p>
          <p>排行：统计商品的销售排行情况</p>
        </div>
      </show-search-grid>
    </div>
    ...
    <div class="chart-area" v-loading="loading">
      <div class="graph">
        <div class="filterSeach">
          <div class="filter-area">
            <el-radio-group @change="indicatorChanged" v-model="indicatorValue" size="mini">
              ...
              <el-radio-button label="salesProductCategory">分类</el-radio-button>
              <el-radio-button label="salesProductTop">排行</el-radio-button>
            </el-radio-group>
          </div>
        ...
        <div class="content" style="width:40%;float:right;line-height:40px;font-size: 13px;padding:100px;height:600px;overflow:auto" v-show="radioValue==='0'&&indicatorValue==='salesProductCategory'">
        ...
        <biz-chart :chartData="chartData" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue!=='salesProductCategory'"></biz-chart>
        <biz-chart :chartData="chartDataOfWork" :loading="loading" :showSearch="false" v-if="radioValue==='0'&&indicatorValue==='salesProductCategory'"></biz-chart>
...
export default {
  ...
  methods: {
    ...
    getGraphData() {
      this.loading = true;
      let url = '';
      if (this.indicatorValue === 'salesProductNum') {
        url = '/admin/ecommerce/kpi/purchase/graph/columnGraph.jhtml';
      } else if (this.indicatorValue === 'salesProductTop') {
      ...
      http
        .post(url, params)
        .then(data => {
          if (this.indicatorValue === 'salesProductCategory') {
            this.countTotal = 0;
          ...
          } else if (this.indicatorValue === 'salesProductNum') {
