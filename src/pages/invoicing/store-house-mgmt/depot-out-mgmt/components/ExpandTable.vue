<template>
  <el-table ref="expandTable" class="expand-table" size="small" tooltip-effect="dark" row-key="id"
    style="width: 100%;min-height: 100%;" height="auto" :border="showBorder" v-loading="loading" :data="data"
    :show-header="showHeader" :rowClassName="setClassName" :cell-class-name="cellClassName" :expand-row-keys="expandedRowKeys"
    @expand-change="expandChangeHandler" @row-click="rowClickHandler" @handleAgent="handleAgent" @handleAdmin="handleAdmin" @quantityChange="quantityChange">
    <el-table-column type="expand" :label="maxLevel" width="100" :class-name="hasChild==='1'?'cell'+layer:'cell0'+layer"
      v-if="isSearch">
      <template slot-scope="scope">
        <expand-table v-if="scope.row.children.length>0" class="expand-table-child" :height="'auto'"
          :hasChild="scope.row.hasChild" :layer="layer + 1" :data="scope.row.children" :show-header="false" :show-border="false"
          :expand-row-keys="expandedRowKeys" @delete="handleDeleteClick" @handleAgent="handleAgent" @handleAdmin="handleAdmin"
          @expand-change="expandChangeHandler" @row-click="rowClickHandler" @quantityChange="quantityChange"></expand-table>
      </template>
    </el-table-column>
    <el-table-column label="仓库名称" prop="name" class-name="first-name">
      <template slot-scope="scope">{{ scope.row.name }}</template>
    </el-table-column>
    <el-table-column label="合计库存" prop="sumStockQuantity">
      <template slot-scope="scope">{{scope.row.sumStockQuantity}}</template>
    </el-table-column>
    <el-table-column label="在库库存" prop="stockQuantity">
      <template slot-scope="scope">{{scope.row.stockQuantity}}</template>
    </el-table-column>
    <el-table-column label="本次数量" prop="">
      <template slot-scope="scope">
        <el-input-number v-model="scope.row.quantity" size="mini" @change="quantityChange" :min="0" :disabled="scope.row.enabled==='0'"></el-input-number>
      </template>
    </el-table-column>
  </el-table>
</template>
...
export default {
  ...
  mounted() {
    ...
    document.querySelectorAll('.expand-table.el-table__body-wrapper')[0].style.height = height + 'px';
    document.querySelectorAll('.expand-table.el-table__body-wrapper')[0].style.overflowX = 'hidden';
    document.querySelectorAll('.expand-table.el-table__body-wrapper')[0].style.overflowY = 'auto';
  },
  updated() {
    this.doLayout();
  },
  methods: {
    ...
    quantityChange(e, row) {
      console.log(0);
      this.$emit('quantityChange', e, row);
    }
  }
};
</script>
