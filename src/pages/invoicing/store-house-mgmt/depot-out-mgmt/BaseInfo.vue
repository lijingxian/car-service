<template>
  <biz-grid class="depot-base-info">
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="120px" size="small" v-loading="loading" :inline-message="true">
      <el-form-item label="单据编号" prop="sn" v-if="this.itemId">
      ...
      <el-form-item label="业务类型" prop="businessType">
        <el-select v-model="form.businessType" placeholder="请选择业务类型" style="width:100%" @change="businessTypeChange" :disabled="!!itemId">
          <el-option label="采购退货" value="0"></el-option>
          <el-option label="销售出库" value="1"></el-option>
          <el-option label="其它出库" value="2"></el-option>
      ...
      <el-form-item label="往来方" prop="relatePartnerType" v-if="form.businessType!=='3'">
        <el-select
          placeholder
          v-model="form.relatePartnerType"
          :disabled="form.businessType === '0' || !!itemId"
        ...
        ></el-input>
        <!-- 仓库 -->
        <el-input v-if="form.relatePartnerType === '3'&&itemId" v-model="form.relatePartnerName" :disabled="!!itemId"></el-input>
      </el-form-item>
      ...
      <el-form-item label="扫码出库备注" prop="memo">
        <el-input :disabled="!!itemId" type="textarea" v-model="form.memo"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    queryDepot() {
      this.loading = true;
      http
        .get(urls.invoicing.depotOutMgmt.shopDepotOut, { shopId: window.top.SHOP_ID || '', id: this.itemId })
        .then(data => {
          this.loading = false;
          // 列表
    ...
    save() {
      ...
      this.$refs.form.validate((valid, a) => {
        if (valid) {
          ...
          if (this.form.id) {
            http
              .put(urls.invoicing.depotOutMgmt.shopDepotOut, this.getSaveParams())
              .then(data => {
            ...
            http
              .post(urls.invoicing.depotOutMgmt.shopDepotOut, this.getSaveParams())
              .then(data => {
                this.$message.success('保存成功');
    ...
    getRelateList() {
      ...
      http
        .get(urls.invoicing.depotOutMgmt.relateBill, params)
        .then(data => {
