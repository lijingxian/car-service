<template>
  <div class="depot-in-mgmt">
    ...
    <biz-flex-table ref="table" :operations="operations" @operate="operate" :filters="filters"
      ...
      @current-change="handleCurrentChange" class="depot-in-mgmt-tabel">
      ...
      <el-table-column label="业务类型" min-width="60px" prop="amount">
        <template slot-scope="scope">
          <div style="width:100%">
            <div class="overflow">
      ...
      <el-table-column prop="date" label="操作" min-width="120">
        <template #default="scope">
          <biz-column-operation :row="scope.row">
            <el-button type="text" size="mini" v-if="(scope.row.state==='0'||scope.row.state==='2')"
              @click.stop="statusChange(scope.row)">确认入库</el-button>
    ...
    <biz-popup-tabs :open="open" @close="open=false" v-model="tabName">
      <el-tab-pane label="基本信息" name="info">
        <base-info :itemId="itemId" :isEdit="isEdit" @setOrderItems="setOrderItems" ref="baseInfo"
          @operationSuccess="createdSuccess"></base-info>
      </el-tab-pane>
      <el-tab-pane label="商品信息" name="productinfo" v-if="isCreate">
        <product-info :itemId="itemId" :isEdit="isEdit" :returnId="returnId" ref="productInfo"
          @operationSuccess="operationSuccess"></product-info>
      </el-tab-pane>
      <el-tab-pane label="入库信息" name="depotinfo" v-if="isCreate">
        <depot-in-info :itemId="itemId" :isEdit="isEdit" ref="depotinfo" @operationSuccess="operationSuccess"
          :tabSelection="tabName"></depot-in-info>
      </el-tab-pane>
      <!-- <el-tab-pane label="发票信息" name="invoiceinfo" v-if="isCreate">
        <invoice-info :itemId="itemId" :isEdit="isEdit" ref="invoiceinfo" @operationSuccess="operationSuccess" :tabSelection="tabName"></invoice-info>
      </el-tab-pane> -->
    </biz-popup-tabs>
    <biz-popup-tabs :open="openPurchaseOrder" @close="openPurchaseOrder=false" v-model="relateBillPurchase">
      <el-tab-pane label="基本信息" name="info">
        <purchase-order-info :orderId="purchaseOrderId" ref="orderinfo" :isRalateBill="isRalateBill"
          @closeTabByEvaluate="closeTabByEvaluate" @closeTabByLogistics="closeTabByLogistics"></purchase-order-info>
      </el-tab-pane>
    </biz-popup-tabs>
    ...
    <biz-popup-tabs :open="openInventory" @close="openInventory=false" v-model="tabInventoryName">
      <el-tab-pane label="基本信息" name="info">
        <inventory-base-info :itemId="inventoryId" :isEdit="isEdit" ref="baseInfo" @operationSuccess="refresh">
        </inventory-base-info>
      </el-tab-pane>
      <el-tab-pane label="商品信息" name="productinfo">
        <inventory-product-info :itemId="inventoryId" :isEdit="isEdit" ref="productinfo" @operationSuccess="refresh">
      ...
      <vue-easy-print tableShow ref="easyPrint">
        <div class="d-p-wrap">
          ...
          <el-table :data="gridData" class="table-class" show-summary size="small">
            ...
            <el-table-column property="fullName" label="商品">
...
import DepotInInfo from './DepotInInfo';
...
export default {
  ...
  computed: {
    ...
    hasAuthComfirm() {
      return this.currentAuth.includes('admin:inport.comfirm');
      // return true;
    },
    hasAuthDel() {
      return this.currentAuth.includes('admin:inport.delete');
      // return true;
    },
    toggleRecommendTxt() {
  ...
  methods: {
    ...
    statusChange(model) {
      ...
      // });
      // if (flag) {
      //   this.$confirm('您本次入库的商品数量小于关联单据的需入库数量，确认入库?', '确认入库？', {
    ...
    confrim(model, state) {
      ...
      http
        .get(urls.invoicing.depotInMgmt.state, params)
        .then(data => {
    ...
    deleteItem(ids) {
      this.$confirm('是否确认删除?', '提示', {
        ...
        this.loading = true;
        http
          .delete(urls.invoicing.depotInMgmt.shopDepotIn, { ids: ids })
          .then(data => {
    ...
    printApply(row) {
      ...
      http
        ...
        .then(data => {
          ...
          this.title = this.getUser().shop.name + '入库单';
          this.form = data.data;
    ...
    async exportInvoice() {
      ...
      try {
        let response = await axios.post('/admin/invoicing/stock/voucher/depotIn/export.jhtml', params, {
          responseType: 'blob'
        });
    ...
    exportStart() {
      ...
      http
        .post('/admin/invoicing/stock/shopDepotIns/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/invoicing/stock/shopDepotIns/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
        console.log('response => ', response);
        const blob = new Blob([response.data], { type: 'application/vnd.ms-excel' });
        console.log(decodeURI(response.headers['content-disposition']));
        const fileName = decodeURI(response.headers['content-disposition'].split('filename=')[1]);
    ...
    getList() {
      ...
      this.loading = true;
      http
        .post(urls.invoicing.depotInMgmt.shopDepotInList, params)
        .then(data => {
          console.log(data);
    ...
    queryKpi() {
      ...
      http
        .get('/admin/invoicing/stock/queryShopDepotInsKpi.jhtml', params)
        .then(data => {
          this.$message({
            customClass: 'motorists-club-activity__kpi',
            showClose: true,
            message: `今日新增${data.data.dayNewAdd}条，待确认入库${data.data.unConfirm}条`,
            duration: 3000
          });
        })
        .catch(ErrorData => {
