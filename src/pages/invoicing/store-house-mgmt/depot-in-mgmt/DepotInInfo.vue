<template>
  <biz-grid class="product-info">
    <biz-list ref="table" :tableData="tableData" :height="'100%'" :loading="loading" :key="Math.random()" v-loading="loading">
      ...
      <el-table-column prop="createDate" label="入库时间"></el-table-column>
      <el-table-column prop="operator" label="入库人"></el-table-column>
    </biz-list>
  </biz-grid>
</template>
...
export default {
  ...
  methods: {
    ...
    query() {
      this.loading = true;
      http
        .get('/admin/invoicing/stock/depotIn/records.jhtml', { id: this.itemId })
        .then(data => {
          this.loading = false;
