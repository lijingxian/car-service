<template>
  ...
  >
    ...
    <el-table-column label="代理商" prop="name" class-name="first-name">
      <template slot-scope="scope">
        <div style="width:100%">
          <div class="overflow">名称：{{scope.row.name?scope.row.name:'--'}}</div>
    ...
    <el-table-column label="电话" prop="phone">
      <template slot-scope="scope">
        <div style="width:100%">
          <div class="overflow">联系人：{{scope.row.contact?scope.row.contact:'--'}}</div>
    ...
    <el-table-column prop="tag" label="操作">
      <template #default="scope">
        <biz-column-operation :row="scope.row">
          <el-button type="text" size="mini" v-if="scope.row.grade!=='2'&&hasAuthAdd" @click.stop="handleAgent($event, scope.row)">添加代理商</el-button>
