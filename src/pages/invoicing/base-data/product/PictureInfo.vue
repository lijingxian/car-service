<template>
  <biz-grid class="commodity-mgmt-picture-info">
    ...
    <el-form ref="form" size="small" :rules="rules" label-position="top" label-width="110px" :disabled="!isEdit" :inline-message="true">
      <el-form-item label="所有图片限制200K以下,建议尺寸3:2">
      </el-form-item>
      <el-form-item label="商品主图" prop="image">
        <BizAvatarUploaderNew v-model="image" @uploadSuccess="uploadSuccess"></BizAvatarUploaderNew>
      </el-form-item>
      <el-form-item label="轮播主图管理">
        <ProductAvatarPictureCardUploader :imageList="carouselImages" :productId="itemId" :isMainPicture='true' @upload-success="handleCarouselUpload"
      ...
      <!-- </el-form-item> -->
      <el-form-item label="商品视频 (请上传视频文件)">
        <ProductVrFileUploader :open="open" :fileList="testDriverList" :productId="itemId" :isTestDrive="true" @upload-success="handleTestDriveUpload"
          @remove-success="handleTestDriveRemove"></ProductVrFileUploader>
      </el-form-item>
      <el-form-item label="商品介绍"></el-form-item>
      <div class="h-editor">
        <biz-editor height="400px" width="450px" v-model="introduction"></biz-editor>
      </div>
      <el-form-item label="购买须知"></el-form-item>
      <div class="h-editor">
        <biz-editor height="400px" width="450px" v-model="notice"></biz-editor>
      </div>
      <el-form-item label="产品售后"></el-form-item>
      <div class="h-editor">
        <biz-editor height="400px" width="450px" v-model="afterSale"></biz-editor>
      </div>
      <el-form-item label="语音播报介绍" label-width="100px"></el-form-item>
      <div class="y-editor">
        <el-input v-model="memo" type="textarea" maxlength="200" :autosize="{ minRows: 5, maxRows: 8 }" width="418px">
        </el-input>
      </div>
      <el-form-item label="内饰图片管理">
        <ProductAvatarPictureCardUploader :imageList="interiorsImages" :productId="itemId" @upload-success="handleInteriorsUpload"
          @remove-success="handleInteriorsRemove">
        </ProductAvatarPictureCardUploader>
      </el-form-item>
      <el-form-item label="空间图片管理">
        <ProductAvatarPictureCardUploader :imageList="spaceImages" :productId="itemId" @upload-success="handleSpaceUpload"
          @remove-success="handleSpaceRemove">
        </ProductAvatarPictureCardUploader>
      </el-form-item>
      <el-form-item label="图解资源管理">
        <ProductAvatarPictureCardUploader :imageList="diagramImages" :productId="itemId" @upload-success="handleDiagramUpload"
      ...
      <el-form-item v-for="item in fullScreenList" :key="item.name">
        <el-tag :closable="!hasFullScreenImages(item)" :disable-transitions="false" type="info" size="warning" @close="handleCloseFullScreen(item)">
          {{item.name}}
        </el-tag>
        <ProductAvatarPictureCardUploader :imageList="item.fullScreenImages" :parentElementName="item.name" @upload-success="handleFullScreenUpload"
          @remove-success="handleFullScreenRemove">
        </ProductAvatarPictureCardUploader>
      </el-form-item>
