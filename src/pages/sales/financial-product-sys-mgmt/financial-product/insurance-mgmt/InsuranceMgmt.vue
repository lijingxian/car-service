<template>
  <div class="insurance-mgmt">
    ...
    >
      <el-table-column width="400" prop="fullName" label="方案名称"></el-table-column>
      <el-table-column prop="financialInstitutionName" label="金融机构">
        <template slot-scope="scope">
          <div style="width:100%">
            <div class="overflow">{{scope.row.financialInstitutionName||'--'}}</div>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="方案来源" prop="planSource">
        <template slot-scope="scope">
          <div style="width:100%">
...
export default {
  ...
  methods: {
    ...
    getKpi() {
      http
        .get('/admin/insurancePlanController/queryInsurancePlanCount.jhtml', {})
        .then(data => {
          this.$message({
            customClass: 'motorists-club-activity__kpi',
            showClose: true,
            message: `已创建保险方案${data.createdCount}个`,
            duration: 3000
          });
        })
        .catch(ErrorData => {
    ...
    getList() {
      ...
      this.loading = true;
      http
        .get('/admin/insurancePlanController/insurancePlanList.jhtml', params)
        .then(data => {
    ...
    clone() {
      ...
      http
        .post('/admin/insurancePlanController/cloneProduct.jhtml', params)
        .then(data => {
          this.$message({
            type: 'success',
            message: '克隆成功!'
    ...
    queryFilters() {
      http
        .get('/admin/financialProductCategory/queryFinancialProductCategoryList.jhtml', {})
        .then(data => {
          data.content.forEach(element => {
            if (element.type === 'carInsurance') {
    ...
    queryFinancialFilters() {
      http
        .get('/admin/financialInstitution/queryFinancialInstitutionList.jhtml', {})
        .then(data => {
