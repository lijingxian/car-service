<template>
  <biz-grid class="lead-base-info">
    <biz-save-button slot="top" :operations="operations" @operate="operate">
    </biz-save-button>
    <el-form ref="form" :rules="rules" size="small" v-loading="loading">
      <div class="radio">
      ...
      <div class='importFile'>
        <el-form-item v-show="showType === '文件导入'">
          <import-file :tableData="importResult" :importLoading="importLoading" :total="total" :disabled="disabled" :count="count"
            :percentage="percentage" @clearTimeOut="clearTimeOut" @restProcess="restProcess" :stepkey="stepkey" :action="'/admin/shopProduct/importProduct.jhtml'"
              ...
              <el-form label-width="120px" size="small">
                <el-form-item label="分类：" prop="topProductCategoryId">
                  <el-select v-model="topProductCategoryId" placeholder="请选择分类" @change="topCategoryChange">
                    <el-option v-for="item in topCategoryOptions" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                  </el-select>
                ...
                <el-form-item label="系列：" v-if="show">
                  <el-checkbox-group v-model="seriesId">
                    <div class="checkbox-group">
                      <el-checkbox v-for="item in seriesOptions" :key="item.id" :label="item.id">
...
export default {
  ...
  methods: {
    ...
    downloadTemplate() {
      window.open('/upload/mode/V服爱车_商品导入模板.xlsx');
    },
    ...
    save() {
      ...
      this.loading = true;
      this.operations[0].disabled = true;
      CommodityMgmtService.importPlatformProduct(
        params,
        data => {
          this.$message({
            type: 'success',
