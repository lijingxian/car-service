  ...
  methods: {
    ...
    getInputCategoryDataList() {
      ...
      CategoryMgmtService.getInputProductCategoryList(
        params,
        data => {
          console.log('[---- dachao ----] [API-从平台导入-查询成功] data=>', data);
          ...
          console.log('[---- dachao ----] [API-从平台导入-查询成功] table=>', table);
        },
    ...
    save() {
      CategoryMgmtService.saveInputProductCategoryList(
        this.getSaveParams(),
        data => {
          console.log('[---- dachao ----] [API-从平台导入-导入成功] data=>', data);
          this.$message.success('导入成功');
          this.$emit('operationSuccess');
        },
        // result_code 不是100的处理
        ErrorData => {
          console.log('[---- dachao ----] [API-从平台导入-导入失败] ErrorData=>', ErrorData);
