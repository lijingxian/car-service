<template>
  <biz-grid class="commodity-mgmt-attribute">
    ...
    <el-table ref="table" :data="attributeDatas" style="width: 100%" size="small" @selection-change="handleSelectChange">
...
export default {
  ...
  methods: {
      ...
      service.getAttributeGroups(
        ...
        data => {
          ...
          this.$nextTick(function() {
            data.attributeDatas.forEach(item => {
              if (item.isCheck === '1') {
    ...
    save() {
      this.attributeDatas.forEach((list, index1) => {
        list.isCheck = '0';
