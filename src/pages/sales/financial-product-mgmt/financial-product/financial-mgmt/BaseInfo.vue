<template>
  <biz-grid class="financial-base-info">
    ...
    <el-form ref='productDataForm' label-width="130px" size="small" v-loading="loading" :model="productDataModel" :rules="productDataRules" :disabled="!isEdit" :inline-message="true">
      ...
      <el-form-item label="金融分类" prop="categoryId">
        <el-select v-model="productDataModel.categoryId" placeholder="请选择金融分类">
      ...
      <el-form-item label="分期方式" prop="method">
        <el-radio-group v-model="productDataModel.method">
          <el-radio label="0" disabled>线上分期</el-radio>
          <el-radio label="1">预约分期</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="利息计算方式" prop="interestRate">
        <el-radio-group v-model="productDataModel.interestRate">
          <el-radio label="0">费率</el-radio>
          <el-radio label="1">利率</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="额度下限" prop="lowerLimit">
        <el-input v-model="productDataModel.lowerLimit">
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item label="额度上限" prop="upperLimit">
        <el-input v-model="productDataModel.upperLimit">
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item label="服务费" prop="serviceCharge">
        <el-input v-model="productDataModel.serviceCharge">
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item label="融资附加" prop="financings">
        <el-checkbox-group v-model="productDataModel.financings">
          <el-checkbox v-for="item in financingGroup" :key="item.id" :label="item.id">{{item.name}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
      ...
      <el-form-item label="商品缩略图：" prop="image">
        <BizAvatarUploaderNew v-model="productDataModel.image">
...
export default {
  ...
  methods: {
    ...
    refresh() {
      ...
      http
        .get('/admin/financialProductPlanController/financialProductPlanBasicInfo.jhtml', params)
        .then(data => {
    ...
    createProduct() {
      ...
      http
        .post('/admin/financialProductPlanController/financialProductPlanBasicInfo.jhtml', params)
        .then(data => {
    ...
    updateProduct() {
      let params = this.getSaveProductDataModel();
      http
        .put('/admin/financialProductPlanController/financialProductPlanBasicInfo.jhtml', params)
        .then(data => {
          this.refresh();
