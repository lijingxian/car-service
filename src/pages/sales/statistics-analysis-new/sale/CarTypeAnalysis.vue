    ...
    <div class="chart-area">
      <div class="graph">
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-if="!isShow">
          <el-table-column prop="xValue" label="车型"></el-table-column>
          <el-table-column prop="yValue" label="销售台次（台）" v-if="filters[0].value==='carTypeOrderProductNum'"></el-table-column>
          <el-table-column prop="yValue" label="销售金额（万元）" v-if="filters[0].value==='carTypeOrderAmountPaid'"></el-table-column>
        </biz-list>
      ...
      filters: [
        {
          type: 'single',
          label: '',
          value: 'carTypeOrderProductNum',
          options: [
            { value: 'carTypeOrderProductNum', label: '销售车型TOP5', type: 'car' },
            { value: 'carTypeOrderAmountPaid', label: '销售金额TOP5', type: 'car' }
          ]
        }
      ],
      kpiName: ['本期', '同比', '环比'],
  ...
  methods: {
    ...
    getGraphData() {
      ...
      http
        ...
        .then(data => {
          if (this.filters[0].value === 'carTypeOrderProductNum') {
            this.chartData = this.dataFormat(data);
          } else {
            this.chartData = this.dataFormatAmount(data);
          }
        })
        .catch(errData => {
          this.loading = false;
    ...
    getTable() {
      ...
      http
        .post('/admin/ecommerce/kpi/sales/table/rank.jhtml', params)
        .then(data => {
    ...
    exportStart() {
      ...
      http
        .post('/admin/ecommerce/kpi/sales/export/rank/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/ecommerce/kpi/sales/export/rank/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
