    ...
    <el-form ref="form" :model="form" label-width="120px" size="small" v-loading="loading" :disabled="!isEdit" :inline-message="true">
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item name="2">
          ...
          <template slot="title" v-if="activeNames.indexOf('2')===-1||!handler">投保信息</template>
          <el-form-item label="意向投保商品">
          ...
          <div v-for="(item,index) of insuranceInfo" :key="index">
            <el-form-item :label="item.name">
              <el-select v-model="form.specificationDatas[0].valueData[index].optionValue" placeholder="请选择">
                <el-option v-for="list in item.datas" :key="list.type" :label="list.name" :value="list.type"></el-option>
              </el-select>
            </el-form-item>
          </div>
        </el-collapse-item>
        <el-collapse-item name="3">
          ...
          <template slot="title" v-if="activeNames.indexOf('3')===-1||!handler">投保信息</template>
          <div v-for="(item,index) in uploadInfo" :key="index">
            <el-form-item :label="item.name">
              ...
              <el-input v-model="form.infoMobile" placeholder="请输入会员电话" v-if="item.name==='电话'"></el-input>
              ...
              <el-input v-model="form.registerDate" placeholder="请输入注册日期" v-if="item.name==='车辆注册日期'"></el-input>
  ...
  methods: {
      ...
      service.viewOrder(
        ...
        data => {
          ...
          this.financingGroup.forEach(item => {
            if (this.form.purchaseTax && item.type === '0') {
              this.form.financings.push(item.type);
            } else if (this.form.vehiclevesselTax && item.type === '1') {
              this.form.financings.push(item.type);
            } else if (this.form.compulsoryInsuranceTax && item.type === '3') {
              this.form.financings.push(item.type);
            } else if (this.form.cardFeeTax && item.type === '2') {
              this.form.financings.push(item.type);
            } else if (this.form.serviceChargeTax && item.type === '999') {
              this.form.financings.push(item.type);
            } else if (this.form.commercialinsurance && item.type === '998') {
              this.form.financings.push(item.type);
            }
          });
    ...
    priceChange(val) {
      this.specInfo.forEach(item => {
        if (item.productData.id === val) {
          this.form.specificationDatas[0].valueData[1].optionValue = item.specificationValueDatas[1].name;
          this.form.specificationDatas[0].valueData[2].optionValue = item.specificationValueDatas[2].name;
        }
      });
      let params = {
        planId: this.form.selectedPlanId,
        productId: this.form.purposeProductId,
        otherTax: this.form.installmentPrice,
        shopId: window.top.SHOP_ID || this.getUser().shop.id
      };
      http
    ...
    seriesProductChange(val) {
      this.productList.forEach((item, index) => {
        if (item.seriesId === val) {
          ...
          this.form.purposeProductId = item.productId;
    ...
    getUploadInfo(productId) {
      ...
      http
        .get('/admin/financialInsuranceProduct/insurancePlanInfo.jhtml', params)
        .then(data => {
    ...
    getInsuranceInfo(productId) {
      let params = {
        planId: productId,
        shopId: window.top.SHOP_ID || this.getUser().shop.id
      };
      http
        .get('/admin/financialInsuranceProduct/insuranceCaculateItemInfo.jhtml', params)
        .then(data => {
          this.insuranceInfo = data.datas;
        })
        .catch(error => {
          if (error.errorMessage) {
            console.log(error.errorMessage);
    ...
    setOrderItems(val) {
      ...
      this.orderItems.forEach(item => {
        ...
        this.form.financialPlanName = item.name;
        this.form.financialPlanId = item.id;
        this.getProductList(item.productId);
        this.getUploadInfo(item.productId);
        this.getInsuranceInfo(item.productId);
      });
      let total = 0;
      val.map(item => {
    ...
    getOrderApply() {
      ...
      http
        .post('/admin/financialInsuranceProduct/applyInsurance.jhtml', params)
        .then(data => {
