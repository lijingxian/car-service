...
export default {
  ...
  watch: {
    dateArray(newVal, oldVal) {
      // console.log('old: ', oldVal, 'new: ', newVal);
      this.selectedDate = newVal.map(date => {
  ...
  methods: {
    ...
    handleTimePick(value, visible, first) {
      if (isDate(value)) {
        const newDate = this.value ? modifyTime(this.date, value.getHours(), value.getMinutes(), value.getSeconds()) : modifyWithTimeString(value, this.defaultTime);
        this.date = newDate;
    ...
    handleDatePick(value) {
      if (this.selectionMode === 'day') {
        this.date = this.value ? modifyDate(this.date, value.getFullYear(), value.getMonth(), value.getDate()) : modifyWithTimeString(value, this.defaultTime);
    ...
    confirm() {
      ...
      // 确认
      if (this.selectionMode === 'dates') {
        // console.log('dates: ', this.selectedDate[0].getFullYear());
        const date = this.selectedDate.map(date => {
