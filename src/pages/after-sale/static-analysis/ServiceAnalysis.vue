    ...
    <div class="chart-area">
      <div class="graph">
        ...
        <biz-list ref="list" :height="height" :tableData="tableData" style="width: 100%" v-if="!isShow">
          ...
          <el-table-column prop="yValue" label="用户满意度" v-if="filters[0].value==='satisfaction'"></el-table-column>
          <el-table-column prop="yValue" label="专业技能" v-if="filters[0].value==='professional'"></el-table-column>
          <el-table-column prop="yValue" label="服务态度" v-if="filters[0].value==='serviceAttitude'"></el-table-column>
          <el-table-column prop="yValue" label="服务速度" v-if="filters[0].value==='serviceSpeed'"></el-table-column>
          <el-table-column prop="yValue" label="售后投诉数" v-if="filters[0].value==='3'"></el-table-column>
          <el-table-column prop="yValue" label="千台车投诉率" v-if="filters[0].value==='4'"></el-table-column>
        </biz-list>
...
export default {
      ...
      filters: [
        {
          ...
          options: [
            { value: '3', label: '售后投诉' },
            { value: '4', label: '千台车投诉率' },
            { value: 'satisfaction', label: '用户满意度' },
            { value: 'professional', label: '专业技能' },
            { value: 'serviceAttitude', label: '服务态度' },
            { value: 'serviceSpeed', label: '服务速度' }
          ]
        }
      ],
      kpiName: ['本期', '同比', '环比'],
  ...
  methods: {
    ...
    getGraphData() {
      this.loading = true;
      let params = {
        type: this.filters[0].value === '4' || this.filters[0].value === '3' ? '5' : 'orderSatisfaction',
      ...
      http
        .post(
          this.filters[0].value === '4' || this.filters[0].value === '3'
            ? '/admin/ecommerce/kpi/afterSales/graph.jhtml'
            : '/admin/ecommerce/kpi/order/graph/order/order.jhtml',
          params
        )
        .then(data => {
          if (this.filters[0].value === '4' || this.filters[0].value === '3') {
    ...
    getTable() {
      let params = {
        type: this.filters[0].value === '4' || this.filters[0].value === '3' ? '5' : 'orderSatisfactionTop',
        orderTypes: ['rescue', 'service'],
        indicator: this.filters[0].value,
        indicators: [this.filters[0].value],
        viewType: this.viewType,
      ...
      http
        .post(
          this.filters[0].value === '4' || this.filters[0].value === '3'
            ? '/admin/ecommerce/kpi/afterSales/table.jhtml'
            : '/admin/ecommerce/kpi/order/table/order/order.jhtml',
          params
        )
        .then(data => {
    ...
    exportStart() {
      ...
      http
        .post('/admin/ecommerce/kpi/afterSales/exportStart.jhtml', params)
        .then(data => {
    ...
    async exportExcle() {
      try {
        let response = await axios.get('/admin/ecommerce/kpi/afterSales/exportResult.jhtml?taskId=' + this.taskId, { responseType: 'blob' });
