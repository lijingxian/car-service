<template>
  <div class="biz-import-file">
    <el-upload :show-file-list="false" :on-success="handleSuccess" :on-error="handleError" :on-progress="handleProgress" :before-upload="handleBeforeUpload"
      :data="data" :name="name" :action="action" accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
    ...
    <el-dialog title="导入中..." class="biz-import-file__uploading-dialog" :visible.sync="uploadingVisible" append-to-body>
    ...
    <el-dialog title="导入结果" class="biz-import-file__importing-dialog" :visible.sync="importingVisible" append-to-body>
      <div class="importing-dialog__content">
        <el-table size="small" ref="table" stripe border :height="400" :data="tableData" tooltip-effect="dark" style="width: 100%" v-loading="importLoading"
        ...
        <div class="info" v-show="showInfo">
          已上传
          <span class="success">{{successCount}}</span> 条，未上传
          <span class="error">{{failCount}}</span> 条
        </div>
      </div>
    </el-dialog>
  </div>
  ...
  .el-dialog {
    ...
    .el-dialog__body {
      ...
      .biz-import-file__uploading-dialog__progress {
        ...
        margin: (($biz-import-file__uploading-dialog-height - 54) / 2 - 12) auto
          0;
