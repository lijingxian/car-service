...
const mutations = {
  ...
  newMessageToClientList(state, payload) {
    ...
    for (let iterator of state.roomList) {
      if (payload.toId === iterator.id) {
        if (iterator.messageList.some(message => (message.id === payload.id || message.timestamp === payload.timestamp))) {
          return;
        }
        if (payload.unshift) {
  ...
  updateUserOnlineList(state, payload) {
    console.log('updateUserOnlineList');
    for (const room of state.roomList) {
      room.isOnline = false;
  ...
  increasePageNumberAndInit(state, payload) {
    console.log('increasePageNumberAndInit');
    for (const room of state.roomList) {
      if (room.imType === payload.imType && room.id === payload.id) {
        room.pageNumber++;
        if (!room.hasInit) {
