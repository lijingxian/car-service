<template>
  <div class="biz-avatar-uploader">
    <el-upload
      ...
      accept="image/*"
      :class="fileData.type==='0'?'avatar-uploader':''"
      :style="fileData.type==='0'?avatarStyle:''"
      name="file"
    ...
    >
      <div style="height: 100%;" v-if="fileData.type==='0'">
        ...
        <div v-show="value" @click.stop="doThis" class="operate">
          ...
          <i @click="upload" class="el-icon-plus" v-if="showHandler"></i>
          <i @click="remove" class="el-icon-delete" v-if="showHandler&&showDelete"></i>
        </div>
      </div>
      <div v-if="fileData.type==='3'" class="avatar">
        <div
          class="name"
          :title="name || value.substring(value.lastIndexOf('/') + 1, value.length)"
        >{{name || value.substring(value.lastIndexOf('/') + 1, value.length)}}</div>
